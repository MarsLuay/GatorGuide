# Transfer Planner Requirement Source Parse Report

Generated: 2026-04-20T08:41:38.043Z

- Primary degree sources parsed: 472
- Parsed successfully: 472
- Parse failures: 0
- Parsed requirement source adapter blocks: 472
- Parsed requirement atom candidates: 13644
- Parsed degree-map block candidates: 1164
- Parsed from cached snapshots after live-source failures: 0
- Parsed from alternate official source URLs: 6
- Owners with parsed UW course codes: 459
- Owners with source-only UW course codes not currently in structured degree-map blocks: 0
- Owners with no parsed UW course codes: 13
- Owners with parser-quality warnings: 76
- Owners with parser-quality notes: 6

## Parser Adapters

- generic-official-html-page: 43
- generic-official-pdf-degree-sheet: 21
- uw-bothell-html-degree-page: 114
- uw-bothell-pdf-worksheet: 3
- uw-seattle-catalog-page: 72
- uw-seattle-html-degree-page: 162
- uw-tacoma-catalog-page: 13
- uw-tacoma-html-degree-page: 44

## Resolution Strategies

- alternate-official-source: 6
- primary-source: 466

## Parser Quality Signals

- alternate-official-source-used: 6
- high-confidence-low-course-coverage: 6
- large-structured-only-course-gap: 42
- material-source-structured-drift: 76

## uw-seattle

### Parser-quality warnings

#### Anthropology

- Source: https://www.washington.edu/students/gencat/program/S/Anthropology-102.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=29; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=29; structured-coverage=37)

#### Anthropology - Anthropology of Globalization

- Source: https://www.washington.edu/students/gencat/program/S/Anthropology-102.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=29; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=29; structured-coverage=37)

#### Anthropology - Archaeological Heritage)

- Source: https://www.washington.edu/students/gencat/program/S/Anthropology-102.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=29; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=29; structured-coverage=37)

#### Anthropology - Archaeology)

- Source: https://www.washington.edu/students/gencat/program/S/Anthropology-102.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=29; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=29; structured-coverage=37)

#### Anthropology - Indigenous Archaeology

- Source: https://www.washington.edu/students/gencat/program/S/Anthropology-102.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=29; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=29; structured-coverage=37)

#### Anthropology - Medical Anthropology and Global Health

- Source: https://www.washington.edu/students/gencat/program/S/Anthropology-102.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=29; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=29; structured-coverage=37)

#### Applied Mathematics - B S to M S pathway

- Source: https://amath.washington.edu/undergraduate-major-applied-mathematics
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=23; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=23; structured-coverage=31)

#### Art

- Source: https://www.washington.edu/students/gencat/program/S/Art+ArtHistory+Design-105.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=19; source-only=0; structured-only=15) | large-structured-only-course-gap (structured-only=15; parsed=19; structured-coverage=34)

#### Art History

- Source: https://www.washington.edu/students/gencat/program/S/Art+ArtHistory+Design-105.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=19; source-only=0; structured-only=15) | large-structured-only-course-gap (structured-only=15; parsed=19; structured-coverage=34)

#### Community, Environment & Planning

- Source: https://www.washington.edu/students/gencat/program/S/UrbanDesignandPlanning-50.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=18; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=18; structured-coverage=27)

#### Design

- Source: https://www.washington.edu/students/gencat/program/S/Art+ArtHistory+Design-105.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=19; source-only=0; structured-only=15) | large-structured-only-course-gap (structured-only=15; parsed=19; structured-coverage=34)

#### Drama - Acting)

- Source: https://www.washington.edu/students/gencat/program/S/Drama-134.html
- Parse confidence: medium
- Quality warnings: material-source-structured-drift (parsed=0; source-only=0; structured-only=6)

#### Earth & Space Sciences

- Source: https://www.washington.edu/students/gencat/program/S/EarthandSpaceSciences-1068.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=18; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=18; structured-coverage=26)

#### Earth & Space Sciences - B.A. route

- Source: https://www.washington.edu/students/gencat/program/S/EarthandSpaceSciences-1068.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=18; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=18; structured-coverage=26)

#### Earth & Space Sciences - B.S. Geology option

- Source: https://www.washington.edu/students/gencat/program/S/EarthandSpaceSciences-1068.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=18; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=18; structured-coverage=26)

#### Earth & Space Sciences - B.S. Geoscience option

- Source: https://www.washington.edu/students/gencat/program/S/EarthandSpaceSciences-1068.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=18; source-only=0; structured-only=8) | large-structured-only-course-gap (structured-only=8; parsed=18; structured-coverage=26)

#### Education Studies - Curriculum and Instruction): Culturally Sustaining Education

- Source: https://www.washington.edu/students/gencat/program/S/CollegeofEducation-351.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=4; source-only=0; structured-only=6) | high-confidence-low-course-coverage (parsed=4; structured-coverage=10; structured-only=6)

#### Education Studies - Curriculum and Instruction): Mathematics Education

- Source: https://www.washington.edu/students/gencat/program/S/CollegeofEducation-351.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=4; source-only=0; structured-only=6) | high-confidence-low-course-coverage (parsed=4; structured-coverage=10; structured-only=6)

#### Education Studies - Curriculum and Instruction): Science Education

- Source: https://www.washington.edu/students/gencat/program/S/CollegeofEducation-351.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=4; source-only=0; structured-only=6) | high-confidence-low-course-coverage (parsed=4; structured-coverage=10; structured-only=6)

#### Environmental Science & Terrestrial Resource Management

- Source: https://www.washington.edu/students/gencat/academic/sefs.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=16; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=16; structured-coverage=25)

#### Environmental Science & Terrestrial Resource Management - Environmental and Forest Sciences and Astrobiology)

- Source: https://www.washington.edu/students/gencat/academic/sefs.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=5; source-only=0; structured-only=12) | large-structured-only-course-gap (structured-only=12; parsed=5; structured-coverage=17) | high-confidence-low-course-coverage (parsed=5; structured-coverage=17; structured-only=12)

#### Environmental Science & Terrestrial Resource Management - Environmental and Forest Sciences)

- Source: https://www.washington.edu/students/gencat/academic/sefs.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=5; source-only=0; structured-only=12) | large-structured-only-course-gap (structured-only=12; parsed=5; structured-coverage=17) | high-confidence-low-course-coverage (parsed=5; structured-coverage=17; structured-only=12)

#### Environmental Science & Terrestrial Resource Management - Natural Resource and Environmental Management

- Source: https://www.washington.edu/students/gencat/academic/sefs.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=16; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=16; structured-coverage=25)

#### Environmental Science & Terrestrial Resource Management - Restoration Ecology and Environmental Horticulture

- Source: https://www.washington.edu/students/gencat/academic/sefs.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=19; source-only=0; structured-only=6)

#### Environmental Science & Terrestrial Resource Management - Sustainable Forest Management

- Source: https://www.washington.edu/students/gencat/academic/sefs.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=19; source-only=0; structured-only=6)

#### Environmental Science & Terrestrial Resource Management - Wildlife Conservation

- Source: https://www.washington.edu/students/gencat/academic/sefs.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=19; source-only=0; structured-only=6)

#### Environmental Studies

- Source: https://www.washington.edu/students/gencat/program/S/ProgramontheEnvironment-1070.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=8; source-only=0; structured-only=39) | large-structured-only-course-gap (structured-only=39; parsed=8; structured-coverage=47) | high-confidence-low-course-coverage (parsed=8; structured-coverage=47; structured-only=39)

#### Ethnomusicology, B.A.

- Source: https://www.washington.edu/students/gencat/program/S/Music-217.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=168; source-only=0; structured-only=6)

#### Ethnomusicology, B.A. - Non Thesis option

- Source: https://www.washington.edu/students/gencat/program/S/Music-217.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=12; source-only=0; structured-only=13) | large-structured-only-course-gap (structured-only=13; parsed=12; structured-coverage=25)

#### Ethnomusicology, B.A. - Thesis option

- Source: https://www.washington.edu/students/gencat/program/S/Music-217.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=12; source-only=0; structured-only=13) | large-structured-only-course-gap (structured-only=13; parsed=12; structured-coverage=25)

#### History & Philosophy of Science

- Source: https://www.washington.edu/students/gencat/program/S/Philosophy-221.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=80; source-only=0; structured-only=25) | large-structured-only-course-gap (structured-only=25; parsed=80; structured-coverage=105)

#### History & Philosophy of Science - Ancient Philosophy)

- Source: https://www.washington.edu/students/gencat/program/S/Philosophy-221.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=80; source-only=0; structured-only=25) | large-structured-only-course-gap (structured-only=25; parsed=80; structured-coverage=105)

#### History & Philosophy of Science - Ethics

- Source: https://www.washington.edu/students/gencat/program/S/Philosophy-221.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=80; source-only=0; structured-only=25) | large-structured-only-course-gap (structured-only=25; parsed=80; structured-coverage=105)

#### Marine Biology

- Source: https://marinebiology.uw.edu/wp-content/uploads/sites/31/2025/11/2025.10.21-Marbiol-Major-Sheet-.pdf
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=31; source-only=0; structured-only=5)

#### Mathematics

- Source: https://math.washington.edu/ba-mathematics-standard-major-requirements-0
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=13; source-only=0; structured-only=11) | large-structured-only-course-gap (structured-only=11; parsed=13; structured-coverage=24)

#### Mathematics - Math option Change

- Source: https://math.washington.edu/ba-mathematics-standard-major-requirements-0
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=23; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=23; structured-coverage=32)

#### Mathematics - Math option Change for Current Math Majors

- Source: https://math.washington.edu/ba-mathematics-standard-major-requirements-0
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=23; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=23; structured-coverage=32)

#### Middle Eastern Languages & Cultures

- Source: https://www.washington.edu/students/gencat/program/S/MiddleEasternLanguagesandCultures-123.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=26; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=26; structured-coverage=35)

#### Middle Eastern Languages & Cultures - Biblical and Ancient Near Eastern Studies

- Source: https://www.washington.edu/students/gencat/program/S/MiddleEasternLanguagesandCultures-123.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=26; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=26; structured-coverage=35)

#### Middle Eastern Languages & Cultures - Comparative Cultures

- Source: https://www.washington.edu/students/gencat/program/S/MiddleEasternLanguagesandCultures-123.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=26; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=26; structured-coverage=35)

#### Middle Eastern Languages & Cultures - Languages and Literatures

- Source: https://www.washington.edu/students/gencat/program/S/MiddleEasternLanguagesandCultures-123.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=26; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=26; structured-coverage=35)

#### Philosophy

- Source: https://www.washington.edu/students/gencat/program/S/Philosophy-221.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=80; source-only=0; structured-only=25) | large-structured-only-course-gap (structured-only=25; parsed=80; structured-coverage=105)

#### Philosophy - Ethics

- Source: https://www.washington.edu/students/gencat/program/S/Philosophy-221.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=80; source-only=0; structured-only=25) | large-structured-only-course-gap (structured-only=25; parsed=80; structured-coverage=105)

### Parsed but no UW course codes found

- Anthropology - Biological Anthropology)
  - Source: https://www.washington.edu/students/gencat/program/S/Anthropology-102.html
  - Parser type: catalog-page
  - Requirement cues found: 1
- Anthropology - Sociocultural Anthropology)
  - Source: https://www.washington.edu/students/gencat/program/S/Anthropology-102.html
  - Parser type: catalog-page
  - Requirement cues found: 1
- Drama - Acting)
  - Source: https://www.washington.edu/students/gencat/program/S/Drama-134.html
  - Parser type: catalog-page
  - Requirement cues found: 5
- Italian
  - Source: https://frenchitalian.washington.edu/undergraduate-studies-italian
  - Parser type: html-degree-page
  - Requirement cues found: 5
- Medical Laboratory Science
  - Source: https://dlmp.uw.edu/education/mls-undergrad
  - Parser type: html-overview-page
  - Requirement cues found: 2

## uw-bothell

### Parser-quality warnings

#### Business Administration (BA) - Accounting Option

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/accounting
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=27; source-only=0; structured-only=5)

#### Business Administration (BA) - Entrepreneurship Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/entrepreneurship
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=19; source-only=0; structured-only=6)

#### Business Administration (BA) - Finance option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/finance-option
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=18; source-only=0; structured-only=5)

#### Business Administration (BA) - Management Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/management
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=18; source-only=0; structured-only=7)

#### Business Administration (BA) - Management Information Systems (MIS) Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/mis
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=26; source-only=0; structured-only=5)

#### Business Administration (BA) - Retail Management Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/retail
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=17; source-only=0; structured-only=7)

#### Business Administration: Accounting (BA) - Accounting Option

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/accounting
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=27; source-only=0; structured-only=5)

#### Business Administration: Accounting (BA) - Entrepreneurship Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/accounting
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=16; source-only=0; structured-only=7)

#### Business Administration: Accounting (BA) - Finance option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/accounting
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=19; source-only=0; structured-only=5)

#### Business Administration: Accounting (BA) - Marketing option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/accounting
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=19; source-only=0; structured-only=5)

#### Business Administration: Finance (BA) - Concentration Courses

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/finance-option
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=48; source-only=0; structured-only=5)

#### Business Administration: Finance (BA) - Finance option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/finance-option
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=18; source-only=0; structured-only=5)

#### Business Administration: Finance (BA) - option Courses

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/finance-option
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=49; source-only=0; structured-only=5)

#### Business Administration: Leadership & Strategic Innovation (BA) - Entrepreneurship Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/leadership
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=23; source-only=0; structured-only=7)

#### Business Administration: Leadership & Strategic Innovation (BA) - Finance option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/leadership
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=22; source-only=0; structured-only=5)

#### Business Administration: Leadership & Strategic Innovation (BA) - Marketing option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/leadership
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=22; source-only=0; structured-only=5)

#### Business Administration: Supply Chain Management (BA) - Accounting Option

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/supply-chain
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=23; source-only=0; structured-only=5)

#### Business Administration: Supply Chain Management (BA) - Finance option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/supply-chain
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=22; source-only=0; structured-only=7)

#### Business Administration: Supply Chain Management (BA) - Marketing option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/supply-chain
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=22; source-only=0; structured-only=7)

#### Business Administration: Supply Chain Management (BA) - Supply Chain Management option

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/supply-chain
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=23; source-only=0; structured-only=5)

#### Business Administration: Supply Chain Management (BA) - Supply Chain Management option School of Business

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration/supply-chain
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=23; source-only=0; structured-only=5)

#### Economics (BS) - Finance option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-economics
- Parse confidence: medium
- Quality warnings: material-source-structured-drift (parsed=2; source-only=0; structured-only=6)

#### Economics (BS) - Marketing option and Concentration

- Source: https://www.uwb.edu/business/undergraduate/bachelor-of-economics
- Parse confidence: medium
- Quality warnings: material-source-structured-drift (parsed=2; source-only=0; structured-only=6)

### Parsed but no UW course codes found

- Business Administration (BA)
  - Source: https://www.uwb.edu/business/undergraduate/bachelor-of-business-administration
  - Parser type: html-overview-page
  - Requirement cues found: 31
- Developmental and Youth Studies (BA) - M.Ed. with Educating for Critical Race Theory (E-Crit) Concentration
  - Source: https://www.uwb.edu/education/undergraduate/developmental-and-youth-studies/degree-requirements
  - Parser type: html-degree-page
  - Requirement cues found: 5
- Developmental and Youth Studies (BA) - M.Ed. with English to Speakers of Other Languages (Esol) Concentration
  - Source: https://www.uwb.edu/education/undergraduate/developmental-and-youth-studies/degree-requirements
  - Parser type: html-degree-page
  - Requirement cues found: 5
- Economics (BS)
  - Source: https://www.uwb.edu/business/undergraduate/bachelor-of-economics
  - Parser type: html-overview-page
  - Requirement cues found: 34

## uw-tacoma

### Parser-quality warnings

#### Arts, Media and Culture (BA) - Film and Media track

- Source: https://www.tacoma.uw.edu/sias/cac/arts-media-culture
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=39; source-only=0; structured-only=6)

#### Arts, Media and Culture (BA) - Visual and Performing Arts track

- Source: https://www.tacoma.uw.edu/sias/cac/arts-media-culture
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=40; source-only=0; structured-only=6)

#### Computer Engineering

- Source: https://www.washington.edu/students/gencat/program/T/SchoolofEngineeringandTechnology-1023.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=119; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=119; structured-coverage=128)

#### Computer Engineering - Bioinformatics Option

- Source: https://www.washington.edu/students/gencat/program/T/SchoolofEngineeringandTechnology-1023.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=119; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=119; structured-coverage=128)

#### Computer Engineering - Create Your Own pathway As A Separate option

- Source: https://www.washington.edu/students/gencat/program/T/SchoolofEngineeringandTechnology-1023.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=119; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=119; structured-coverage=128)

#### Computer Engineering - Mobile Digital Forensics

- Source: https://www.washington.edu/students/gencat/program/T/SchoolofEngineeringandTechnology-1023.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=119; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=119; structured-coverage=128)

#### Electrical Engineering

- Source: https://www.washington.edu/students/gencat/program/T/SchoolofEngineeringandTechnology-1023.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=119; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=119; structured-coverage=128)

#### Electrical Engineering - Bioinformatics Option

- Source: https://www.washington.edu/students/gencat/program/T/SchoolofEngineeringandTechnology-1023.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=119; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=119; structured-coverage=128)

#### Electrical Engineering - Create Your Own pathway As A Separate option

- Source: https://www.washington.edu/students/gencat/program/T/SchoolofEngineeringandTechnology-1023.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=119; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=119; structured-coverage=128)

#### Electrical Engineering - Mobile Digital Forensics

- Source: https://www.washington.edu/students/gencat/program/T/SchoolofEngineeringandTechnology-1023.html
- Parse confidence: high
- Quality warnings: material-source-structured-drift (parsed=119; source-only=0; structured-only=9) | large-structured-only-course-gap (structured-only=9; parsed=119; structured-coverage=128)

### Parsed but no UW course codes found

- Arts, Media and Culture (BA)
  - Source: https://www.tacoma.uw.edu/sias/cac/arts-media-culture
  - Parser type: html-overview-page
  - Requirement cues found: 10
- Communications (BA) - PROFESSIONAL TRACK
  - Source: https://www.tacoma.uw.edu/sias/cac/communication
  - Parser type: html-degree-page
  - Requirement cues found: 5
- History (BA)
  - Source: https://www.tacoma.uw.edu/sias/socs/history
  - Parser type: html-overview-page
  - Requirement cues found: 24
- Social Welfare (BA)
  - Source: https://www.tacoma.uw.edu/swcj/basw-curriculum
  - Parser type: html-curriculum-page
  - Requirement cues found: 18

