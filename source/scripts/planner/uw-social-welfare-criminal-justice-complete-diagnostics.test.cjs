...
} = require("./fixtures/uw-social-welfare-criminal-justice-complete-diagnostics.fixture.cjs");

const RUN_DIAGNOSTICS =
  process.env.TRANSFER_PLANNER_RUN_UW_SOCIAL_WELFARE_CRIMINAL_JUSTICE_DIAGNOSTICS === "1";
...
function normalizeCourseCode(value) {
  return String(value ?? "")
    ...
    .replace(/\bT\s+(BUS|CRIM|HLTH|INFO|LAW|PHIL|PSYCH|SOC|SOCWF|URB)\b/g, "T$1")
    .replace(/\s+/g, " ")
    .trim()
...
function getDirectSocialWelfareCriminalJusticePlanIds() {
  const planner = getPlanner();
...
test("UW social welfare/criminal justice complete diagnostic fixture is source scoped and UW-course only", () => {
  assert.deepEqual(
    socialWelfareCriminalJusticePrograms.map((program) => program.planId),
    [
  ...
  for (const program of socialWelfareCriminalJusticePrograms) {
...
diagnosticTest("UW social welfare/criminal justice target roster matches direct official social welfare and criminal justice majors", () => {
  const expectedPlanIds = uniqueSorted(socialWelfareCriminalJusticePrograms.map((program) => program.planId));
  const actualPlanIds = getDirectSocialWelfareCriminalJusticePlanIds();

  assert.deepEqual(
    actualPlanIds,
    expectedPlanIds,
    [
      "Direct Social Welfare/Criminal Justice roster should include Seattle Social Welfare, Tacoma Social Welfare, and Tacoma Criminal Justice.",
      "Seattle Law, Societies & Justice, Sociology, and Community, Environment & Planning are sibling social-science programs and should stay outside this target fixture.",
