...
function normalizeCourseCode(value) {
  return String(value ?? "")
    ...
    .replace(/\bB\s+(BECN|BUS|MATH|WRIT)\b/g, "B$1")
    .replace(/\bIND\s+E\b/g, "INDE")
    .replace(/\bQ\s+(METH|SCI)\b/g, "Q$1")
    .replace(/\bST\s+MATH\b/g, "STMATH")
    .replace(/\bT\s+(BECON|ECON|MATH|PHIL|POLS|WRT)\b/g, "T$1")
    .replace(/\s+/g, " ")
    .trim()
...
function getDirectEconomicsPlanIds() {
  const planner = getPlanner();
  return uniqueSorted(
      ...
      planner
        ...
        .filter((plan) => {
          ...
          return titles.some((value) =>
            /^(?:Economics(?:\s*\(BS\))?|Economics and Policy Analysis(?:\s*\(BA\))?)(?:\s|:|\(|$)/i.test(value)
          );
        })
        .map((plan) => plan.id)
    )
...
test("UW economics complete diagnostic fixture is source scoped and UW-course only", () => {
  assert.deepEqual(
    economicsPrograms.map((program) => program.planId),
    [
  ...
  for (const program of economicsPrograms) {
...
diagnosticTest("UW economics target roster matches direct official economics majors", () => {
  const expectedPlanIds = uniqueSorted(economicsPrograms.map((program) => program.planId));
  const actualPlanIds = getDirectEconomicsPlanIds();

  assert.deepEqual(
    actualPlanIds,
    expectedPlanIds,
    [
      "Direct Economics roster should include Seattle Economics, Bothell Economics, and Tacoma Economics and Policy Analysis only.",
      "Bothell Law, Economics & Public Policy and Tacoma Politics, Philosophy and Economics are sibling programs and should stay outside this target fixture.",
