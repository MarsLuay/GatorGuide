...
const EXPLICIT_COURSE_CODE_PATTERN =
  /\b([A-Z&][A-Za-z&]{0,19}(?:\s+[A-Z&][A-Za-z&]{0,19})?)\s*(\d{3}[A-Za-z]?)\b/g;
...
const PARSED_PRIMARY_REQUIREMENT_HEADING_PATTERN =
  /\b(?:(?:undergraduate|bachelor(?:'s)?)\s+)?(?:major|degree|program)\s+(?:and\s+minor\s+)?requirements?\b|\b(?:required|core)\s+(?:courses?|coursework)\b/i;

function shouldPromotePrimaryOverviewFromParsedEvidence(
  entry,
  parsedSourceRole,
  metadataSourceRole,
  parsed
  ...
  const resolvedSourceRole = parsedSourceRole ?? metadataSourceRole;
  if (resolvedSourceRole !== "overview") {
    return false;
  }
  ...
  const requirementEvidenceLines = [
    ...(parsed?.headings ?? []),
    ...(parsed?.requirementCueLines ?? []),
    ...(parsed?.chooseStatements ?? []),
  ];
  return requirementEvidenceLines.some((line) =>
    PARSED_PRIMARY_REQUIREMENT_HEADING_PATTERN.test(normalizeWhitespace(line))
  );
}
...
function classifyRequirementSourceRole(entry) {
  ...
  if (manifestRole === "overview") {
    return "overview";
  }
  const isUwbMajorAdmissionsPage =
    /uwb\.edu\/[^?#]*\/majors\/[^/?#]+\/admissions\/?(?:[?#].*)?$/i.test(entryUrl);
  if (isUwbMajorAdmissionsPage) {
...
const ADMISSION_PREPARATION_SOURCE_ROLES = new Set([
  "admission-prerequisite-source",
...
function shouldAllowAlternateManifestEntryForParse(entry, alternateEntry) {
  ...
  if (ADMISSION_PREPARATION_SOURCE_ROLES.has(sourceRole)) {
    return ADMISSION_PREPARATION_SOURCE_ROLES.has(alternateRole);
  }
...
function extractExplicitCourseCodesFromLine(line) {
  ...
  for (let index = 0; index < explicitMatches.length; index += 1) {
    ...
    extractedCourseCodes.push(
      ...extractHyphenatedCourseChainContinuations(subject, match[2], trailingSegment)
    );
...
function isCourseCodeMatchImmediatelyFollowedByLevel(line, match) {
  ...
  return /^\s*(?:(?:-\s*)?level\b|\+(?=\s*(?:(?:level|courses?)\b|$)))/i.test(
    trailingText
  );
}

function extractHyphenatedCourseChainContinuations(subject, firstNumber, trailingSegment) {
  const numbers = [String(firstNumber ?? "").toUpperCase()];
  let remaining = String(trailingSegment ?? "");

  while (remaining) {
    const match = remaining.match(/^\s*[-\u2013\u2014]\s*(\d{3}[A-Za-z]?)/);
    if (!match) {
      break;
    }
    numbers.push(match[1].toUpperCase());
  ...
  let continuationNumbers = [];
  if (numbers.length >= 3) {
    continuationNumbers = numbers.slice(1);
  } else {
    const [startNumber, endNumber] = numbers;
    const startNumeric = Number.parseInt(startNumber, 10);
    const endNumeric = Number.parseInt(endNumber, 10);
    const hasLetterSuffix = /[A-Z]$/.test(startNumber) || /[A-Z]$/.test(endNumber);
    const distance = Math.abs(endNumeric - startNumeric);

    if (hasLetterSuffix && startNumeric === endNumeric) {
      continuationNumbers = [endNumber];
    } else if (!hasLetterSuffix && distance > 0 && distance <= 2) {
      const direction = endNumeric > startNumeric ? 1 : -1;
      for (
        let numeric = startNumeric + direction;
        numeric !== endNumeric + direction;
        numeric += direction
      ) {
        continuationNumbers.push(String(numeric).padStart(3, "0"));
      }
    }
  }

  return continuationNumbers
    .map((number) => normalizeExtractedCourseCode(subject, number))
    .filter(Boolean);
}

function normalizeCourseConnectorSpacing(line) {
  return String(line ?? "").replace(
    /\b([A-Za-z&]+(?:\s+[A-Za-z&]+)*\s+\d{3})(?=(?:or|and)\b)/gi,
    "$1 "
  );
}

function normalizeKnownSingleLetterSpacedSubjects(line) {
  return String(line ?? "").replace(
    /\b([A-Za-z])\s+([A-Za-z&]{2,})(?=\s+\d{3}[A-Za-z]?\b)/g,
    (match, leadingToken, trailingToken) => {
      const spacedSubject = `${leadingToken} ${trailingToken}`.toUpperCase();
      const collapsedSubject = `${leadingToken}${trailingToken}`.toUpperCase();
      return !KNOWN_UW_EXTRACTED_COURSE_SUBJECTS.has(spacedSubject) &&
...
function extractCourseCodesFromLine(line) {
  const normalizedLine = normalizeWhitespace(
    normalizeKnownSingleLetterSpacedSubjects(normalizeCourseConnectorSpacing(line))
  );
...
function isDirectCourseLineRecoverySource(entry) {
  ...
  return (
    ...
    (/\bhtml-admissions-page\b/i.test(sourceText) &&
      /\b(?:admissions?|prereq(?:uisites?)?|requirements?|preparation)\b/i.test(sourceText)) ||
...
function extractCourseCodesFromLines(lines, headings, entry = null) {
  return filterExplicitlyExcludedCourseCodesFromLines(
    lines,
    [
...
function lineBelongsToDegreeRouteComparisonSection(lines, hintLineIndex) {
  ...
  return !previousLines
    ...
    .some((line) => isDegreeRouteComparisonSectionTerminator(line));
}
...
function isDegreeRouteComparisonSectionTerminator(line) {
  ...
  return (
    /\blearn\s+more\s+about\s+b\.?\s*s\.?\s+in\b/i.test(normalizedLine) ||
    /^(?:b\.?\s*[as]\.?(?:\s+in\s+.{1,80})?\s+requirements?|(?:degree|major|completion)\s+requirements?)\b/i.test(
      normalizedLine
    )
  );
}
    ...
    hintLineIndex >= 0 &&
      (hasRequirementContextNearLine(lines, hintLineIndex, 4) ||
        (lines ?? [])
          ...
          .some((line) => PROGRAMMING_CHOICE_CONTEXT_PATTERN.test(stripChoiceListLine(line))));
  if (
...
function buildCourseCodeMentionPattern(courseCode) {
  const normalizedCourseCode = normalizeCourseCode(courseCode);
  const match = normalizedCourseCode.match(/^([A-Z&]+(?:\s+[A-Z&]+)*)\s+(\d{3}[A-Z]?)$/);
  if (!match) {
    return null;
  }

  const subjectPattern = match[1]
    .replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
    .replace(/\s+/g, "\\s+");
  const numberPattern = match[2].replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return `\\b${subjectPattern}\\s+${numberPattern}\\b`;
}

function lineExplicitlyExcludesCourseCode(line, courseCode) {
  const normalizedLine = normalizeWhitespace(stripChoiceListLine(line));
  const coursePattern = buildCourseCodeMentionPattern(courseCode);
  if (!normalizedLine || !coursePattern) {
    return false;
  }

  const noExceptMeansAllowed = new RegExp(
    `\\bno\\b[^.;:]{0,80}\\bexcept\\b[^.;:]{0,100}${coursePattern}`,
    "i"
  ).test(normalizedLine);
  if (
    !noExceptMeansAllowed &&
    new RegExp(
      `\\b(?:except|excluding|with\\s+the\\s+exception\\s+of)\\b[^.;:]{0,100}${coursePattern}`,
      "i"
    ).test(normalizedLine)
  ) {
    return true;
  ...
  return new RegExp(
    `${coursePattern}[^.;:]{0,120}\\b(?:excluded|excludes|not\\s+(?:accepted|allowed|counted?|substitut(?:e|es|ed|ing)|satisf(?:y|ies)|fulfill(?:s|ed)?|meet)|cannot\\s+(?:count|receive\\s+credit|be\\s+(?:used|applied)|substitut(?:e|es?)|satisfy|fulfill|meet)|may\\s+not\\s+(?:count|receive\\s+credit|be\\s+(?:used|applied)|substitut(?:e|es?)|satisfy|fulfill|meet)|will\\s+not\\s+(?:count|receive\\s+credit|be\\s+(?:used|applied)|substitut(?:e|es?)|satisfy|fulfill|meet)|does\\s+not\\s+(?:count|satisfy|fulfill|meet)|do\\s+not\\s+count)\\b`,
    "i"
  ).test(normalizedLine);
}

function getExplicitlyExcludedCourseCodesFromLines(lines) {
  return uniqueSorted(
    (lines ?? [])
      .flatMap((line) =>
        extractCourseCodesFromLine(line).filter((courseCode) =>
          lineExplicitlyExcludesCourseCode(line, courseCode)
        )
      )
      .map((courseCode) => normalizeCourseCode(courseCode))
      .filter(Boolean)
...
function filterExplicitlyExcludedCourseCodes(courseCodes, excludedCourseCodeSet) {
  if (!excludedCourseCodeSet?.size) {
    return uniqueSorted(courseCodes ?? []);
  }
  return uniqueSorted(
    (courseCodes ?? []).filter(
      (courseCode) => !excludedCourseCodeSet.has(normalizeCourseCode(courseCode))
    )
  );
}

function filterExplicitlyExcludedCourseCodesFromLines(lines, courseCodes) {
  return filterExplicitlyExcludedCourseCodes(
    courseCodes,
    new Set(getExplicitlyExcludedCourseCodesFromLines(lines))
  );
}

function getGloballyExplicitlyExcludedCourseCodes(lines, auditRows) {
  const excludedCourseCodes = getExplicitlyExcludedCourseCodesFromLines(lines);
  if (!excludedCourseCodes.length) {
    return [];
  }

  const positiveSchedulableEvidenceCourseCodes = new Set();
  for (const row of auditRows ?? []) {
    if (row?.schedulable !== true) {
      continue;
    }

    const rawLine = normalizeWhitespace(row.rawLine);
    for (const courseCode of row.courseCodesExtracted ?? []) {
      const normalizedCourseCode = normalizeCourseCode(courseCode);
      if (
        normalizedCourseCode &&
        !lineExplicitlyExcludesCourseCode(rawLine, normalizedCourseCode)
      ) {
        positiveSchedulableEvidenceCourseCodes.add(normalizedCourseCode);
      }
    }
  }

  return excludedCourseCodes.filter(
    (courseCode) => !positiveSchedulableEvidenceCourseCodes.has(normalizeCourseCode(courseCode))
  );
}

function extractCourseCodesFromRequirementLine(line) {
  const normalizedLine = normalizeWhitespace(line);
  const historicalAliasCourseCodes = getHistoricalAliasCourseCodes(normalizedLine);
  return uniqueSorted(
    extractCourseCodesFromLine(normalizedLine).filter(
      (courseCode) =>
        !isCourseCodeLevelReference(normalizedLine, courseCode) &&
        !lineExplicitlyExcludesCourseCode(normalizedLine, courseCode) &&
        !historicalAliasCourseCodes.has(normalizeCourseCode(courseCode))
    )
  );
}

function getHistoricalAliasCourseCodes(line) {
  const normalizedLine = normalizeWhitespace(line);
  const historicalClauses = [];

  for (const match of normalizedLine.matchAll(
    /\([^)]*\b(?:formerly|previously|renumbered)\b[^)]*\)/gi
  )) {
    historicalClauses.push(match[0]);
  }
  for (const match of normalizedLine.matchAll(
    /\b(?:formerly|previously)(?:\s+known\s+as)?\b[^.;)]*/gi
  )) {
    historicalClauses.push(match[0]);
  }
  for (const match of normalizedLine.matchAll(/\brenumbered\s+from\b[^.;)]*/gi)) {
    historicalClauses.push(match[0]);
  }

  const renumberedToIndex = normalizedLine.search(/\b(?:was\s+)?renumbered\s+(?:as|to)\b/i);
  if (renumberedToIndex > 0) {
    const priorCodes = extractCourseCodesFromLine(normalizedLine.slice(0, renumberedToIndex));
    if (priorCodes.length) {
      historicalClauses.push(priorCodes.at(-1));
    }
  }

  return new Set(
    historicalClauses
      .flatMap((clause) => extractCourseCodesFromLine(clause))
      .map((courseCode) => normalizeCourseCode(courseCode))
      .filter(Boolean)
  );
}
...
function hasUndergraduateRequirementResetCue(text) {
  const normalizedText = normalizeWhitespace(text);
  return (
    /\b(?:curriculum|degree options?|major options?|courses?\s+for\s+graduation|core courses?)\b/i.test(
      normalizedText
    ) && !GRADUATE_DEGREE_CONTEXT_PATTERN.test(normalizedText)
  );
}
...
function hasNonRequirementCourseExclusionCue(text) {
  const normalizedText = normalizeWhitespace(text);
  if (!normalizedText || !extractCourseCodesFromLine(normalizedText).length) {
    return false;
  }

  return (
    /\b(?:will\s+not|cannot|can't|does\s+not|do\s+not|may\s+not|not)\s+(?:substitut(?:e|es?|ed|ing)|count|receive\s+credit|be\s+(?:used|applied)|satisfy|fulfill|meet|apply)\b/i.test(
      normalizedText
    ) ||
    /\b(?:do|does|will|can|could|may)\b.{0,80}\b(?:not|n't)\b.{0,80}\b(?:count|substitut(?:e|es?|ed|ing)|receive\s+credit|satisfy|fulfill|meet|apply)\b/i.test(
      normalizedText
    )
  );
}

function hasCourseQuestionSupportCue(text) {
  const normalizedText = normalizeWhitespace(text);
  return (
    /\?/.test(normalizedText) &&
    !sourceLineStartsWithCourseCode(normalizedText) &&
    extractCourseCodesFromLine(normalizedText).length > 0
  );
}

function hasHonorsOnlySupportCue(text) {
  const normalizedText = normalizeWhitespace(text);
  if (!normalizedText) {
    return false;
  }
  ...
  return (
    /^(?:[A-Z& ]+\s+)?honou?rs$/i.test(normalizedText) ||
    /\b(?:with\s+honors|honors\s+designation|honors\s+thesis|honors\s+requirements?|program\s+honors|to\s+qualify\s+for\b.{0,80}\bhonors|pursue\s+honors)\b/i.test(
      normalizedText
    ) &&
    !/\b(?:regular|standard|accelerated)\b.{0,60}\bor\b.{0,60}\bhonors?\b|\bhonors?\b.{0,60}\bor\b.{0,60}\b(?:regular|standard|accelerated|equivalent)\b/i.test(
      normalizedText
    )
  );
}

function hasAcademicSupportOnlyCourseCue(text) {
  const normalizedText = normalizeWhitespace(text);
  if (!normalizedText || !extractCourseCodesFromLine(normalizedText).length) {
    return false;
  }
  ...
  return (
    /\boptional\b/i.test(normalizedText) ||
    /\bworkshop\s+style\s+course\b/i.test(normalizedText) ||
    /\b(?:help|helps)\s+(?:you|students?)\s+(?:solve|develop|prepare|review|practice)\b/i.test(
      normalizedText
    )
  );
}

function isInheritedHonorsSupportSection(inheritedRole, inheritedRoleReason) {
  return (
    inheritedRole === "support-metadata" &&
    /\bhonors-only\b/i.test(String(inheritedRoleReason ?? ""))
  );
}
...
function parsePriorAdmitEra(value) {
  const normalizedText = normalizeWhitespace(String(value ?? ""))
    .replace(/[_:-]+/g, " ")
    .toLowerCase();
  const match = normalizedText.match(
    /\b(?:pre|before|prior\s+to)\s+(spring|summer|autumn|fall|winter)\s+(\d{4})\b/i
  );
  if (!match) {
    return null;
  }
  return {
    term: match[1].toLowerCase() === "fall" ? "autumn" : match[1].toLowerCase(),
    year: Number.parseInt(match[2], 10),
  };
}

function getPriorAdmitCurriculumOwnerEra(entry) {
  return parsePriorAdmitEra(
    `${entry?.ownerId ?? ""} ${entry?.ownerTitle ?? ""} ${entry?.pathwayId ?? ""} ${entry?.label ?? ""}`
  );
}

function historicalRequirementSectionMatchesOwner(entry, text) {
  const ownerEra = getPriorAdmitCurriculumOwnerEra(entry);
  const sectionEra = parsePriorAdmitEra(text);
  return (
    ownerEra !== null &&
    sectionEra !== null &&
    ownerEra.term === sectionEra.term &&
    ownerEra.year === sectionEra.year
  );
}

function hasCurrentCurriculumBoundaryCue(text) {
  const normalizedText = normalizeWhitespace(text);
  return (
    /^(?:current|new)\s+(?:degree\s+)?(?:curriculum|requirements?)\b/i.test(normalizedText) ||
    /^(?:curriculum|requirements?)\s+(?:for\s+)?(?:current|new)\s+students?\b/i.test(
      normalizedText
    ) ||
    /\b(?:beginning|effective)\s+(?:in\s+)?(?:spring|summer|autumn|fall|winter)\s+\d{4}\b/i.test(
      normalizedText
    )
  );
}
...
function isInheritedHistoricalRequirementSection(inheritedRole, inheritedRoleReason, text) {
  return (
    ...
    !hasCurrentCurriculumBoundaryCue(text) &&
    !isUndergraduateCatalogSectionResetLine(text)
  );
}

function classifySourceSectionRoleForLine(
  line,
  inheritedRole = null,
  inheritedRoleReason = null,
  sourceOwner = null
) {
  ...
  if (hasHistoricalRequirementSectionCue(text)) {
    if (historicalRequirementSectionMatchesOwner(sourceOwner, text)) {
      return {
        sectionRole: "primary-requirement-section",
        reason: "historical requirement section matches prior-admit curriculum owner",
      };
    }
    return {
  ...
  if (courseCodes.length === 0 && hasHonorsOnlySupportCue(text)) {
    return {
      sectionRole: "support-metadata",
      reason: "honors-only section is support metadata",
    };
  }

  if (
    isInheritedHonorsSupportSection(inheritedRole, inheritedRoleReason) &&
    !hasAdmissionPrepHeadingCue(text) &&
    !hasUndergraduateRequirementResetCue(text) &&
    !isUndergraduateCatalogSectionResetLine(text)
  ) {
    return {
      sectionRole: "support-metadata",
      reason: "inherits honors-only support section",
    };
  }
  ...
  if (hasCourseQuestionSupportCue(text)) {
    return {
      sectionRole: "support-metadata",
      reason: "course-bearing question line is support metadata",
    };
  }

  if (hasAcademicSupportOnlyCourseCue(text)) {
    return {
      sectionRole: "support-metadata",
      reason: "optional or academic-support course row is support metadata",
    };
  }

  if (hasNonRequirementCourseExclusionCue(text)) {
    return {
      sectionRole: "support-metadata",
      reason: "negative course substitution or credit note is support metadata",
    };
  }
  ...
  if (prerequisiteCue) {
    const colonLabel = text.includes(":") ? text.slice(0, text.indexOf(":")) : "";
    if (
      sourceLineStartsWithCourseCode(text) &&
      /^[^:]{2,120}:\s+\S/.test(text) &&
      !/\bprereq(?:uisite)?s?\b/i.test(colonLabel) &&
      ...
      return {
        sectionRole: "primary-requirement-section",
        reason: "labeled requirement course row with inline prerequisite note",
      };
    }
...
function shouldInheritGraduateRequirementSection(currentRole, currentRoleReason, line) {
  return (
    ...
    !hasUndergraduateRequirementResetCue(line) &&
    !isUndergraduateCatalogSectionResetLine(line)
  );
}

function isTacomaRnBsnSampleProgramPlanSource(sourceUrl) {
  return /\/\/www\.tacoma\.uw\.edu\/nursing\/rn-bsn-sample-program-plans(?:$|[?#/])/i.test(
    String(sourceUrl ?? "")
  );
}

function isTacomaRnBsnSampleProgramPlanResetLine(sourceUrl, line) {
  return (
    isTacomaRnBsnSampleProgramPlanSource(sourceUrl) &&
    /\b(?:rn-bsn\s+sample\s+program\s+plans?|sample\s+program\s+plans?\s+for\s+(?:summer|autumn)\s+cohort)\b/i.test(
...
function buildParserPrerequisiteFilterAuditRows(input) {
  ...
  for (let index = 0; index < (input.snapshotLines ?? []).length; index += 1) {
    ...
    if (
      isUndergraduateCatalogSectionResetLine(normalizedLine) ||
    ...
    const lineIsGraduateRequirementContext =
      !isTacomaRnBsnSampleProgramPlanResetLine(input.sourceUrl, normalizedLine) &&
      isGraduateOrAppliedMastersRequirementContext(
    ...
    const explicit = lineIsGraduateRequirementContext
      ...
      : inDegreeRouteComparisonSection
        ...
        : shouldInheritGraduateRequirementSection(currentRole, currentRoleReason, normalizedLine)
          ...
          : classifySourceSectionRoleForLine(
              normalizedLine,
              currentRole,
              currentRoleReason,
              input
    ...
    if (
      inDegreeRouteComparisonSection &&
      isDegreeRouteComparisonSectionTerminator(normalizedLine)
    ) {
...
function getSchedulableSourceSectionLines(snapshotLines, auditRows) {
  const decisionBySourceLineIndex = new Map(
    (auditRows ?? [])
      .filter((row) => Number.isInteger(row.sourceLineIndex))
      .map((row) => [row.sourceLineIndex, row.schedulable === true])
  );
  return (snapshotLines ?? []).filter((_, index) => decisionBySourceLineIndex.get(index) !== false);
}
...
function getGroupSourceSectionDecision(group, auditRows) {
  ...
  if (group.detectedOptionCue === "standalone course-count choice list") {
    return {
      detectedSectionRole: "primary-requirement-section",
      schedulable: true,
      reason: "standalone explicit course-count choice requirement",
    };
  }
...
function stripNonOptionCourseEvidenceFromChoiceLine(value) {
  let text = normalizeWhitespace(
    String(value ?? "")
      ...
      .replace(/\([^)]*\b(?:pr|pre-?req|prereq(?:uisite)?)[^)]*\)/gi, "")
      .replace(
        /\([^)]*\b(?:may\s+be\s+used|may\s+satisfy|may\s+count|cannot\s+count|but\s+not\s+both|not\s+both)\b[^)]*\)/gi,
        ""
      )
...
function isStandaloneCreditAmountLine(line) {
  return /^\d+(?:-\d+)?\s*cr$/i.test(normalizeWhitespace(stripChoiceListLine(line)));
}

function lineStartsParentheticalChoiceList(line) {
  const normalizedLine = normalizeWhitespace(stripChoiceListLine(line));
  if (!/^\(/.test(normalizedLine)) {
    return false;
  }
  if (CHOICE_LIST_START_PATTERN.test(normalizedLine)) {
    return true;
  }
  return /\bor\b/i.test(normalizedLine) && extractCourseCodesFromLine(normalizedLine).length >= 2;
}

function buildForwardLabeledChoiceRequirementLines(snapshotLines) {
  const choiceLines = [];

  for (let labelIndex = 0; labelIndex < (snapshotLines ?? []).length; labelIndex += 1) {
    const label = normalizeWhitespace(stripLeadingRequirementGlyphs(stripChoiceListLine(snapshotLines[labelIndex])));
    if (!looksLikeStandaloneRequirementTitleLine(label)) {
      continue;
    }
    if (!hasChoiceRequirementContext(label) && !PROGRAMMING_CHOICE_CONTEXT_PATTERN.test(label)) {
      continue;
    }

    const creditLine = isStandaloneCreditAmountLine(snapshotLines[labelIndex + 1] ?? "")
      ? normalizeWhitespace(stripChoiceListLine(snapshotLines[labelIndex + 1]))
      : "";
    const requirementLabel = normalizeWhitespace(`${label} ${creditLine}`);
    const scanLimit = Math.min((snapshotLines ?? []).length, labelIndex + 12);
    for (let choiceIndex = labelIndex + 1; choiceIndex < scanLimit; choiceIndex += 1) {
      const candidate = normalizeWhitespace(stripChoiceListLine(snapshotLines[choiceIndex]));
      if (!candidate) {
        continue;
      }
      if (!CHOICE_LIST_START_PATTERN.test(candidate) && !lineStartsParentheticalChoiceList(candidate)) {
        continue;
      }
      choiceLines.push(
        buildChoiceListSourceLine(requirementLabel, snapshotLines[choiceIndex], snapshotLines, choiceIndex)
      );
      break;
    }
  }
...
function buildMultiLineChoiceRequirementLines(snapshotLines) {
  ...
  choiceLines.push(...buildForwardLabeledChoiceRequirementLines(snapshotLines));
...
function buildForwardLabeledChoiceRequirementGroups(owner, parsedCourseCodes, snapshotLines) {
  const parsedCourseCodeSet = new Set(parsedCourseCodes.map((courseCode) => normalizeCourseCode(courseCode)));
  const groups = [];
  const seenGroupIds = new Set();

  for (const sourceLine of buildForwardLabeledChoiceRequirementLines(snapshotLines)) {
    const normalizedLine = normalizeWhitespace(stripChoiceListLine(sourceLine));
    const optionExtractionLine = stripNonOptionCourseEvidenceFromChoiceLine(normalizedLine);
    const courseCodes = uniqueInOrder(
      extractCourseCodesFromRequirementLine(optionExtractionLine)
        .map(normalizeCourseCode)
        .filter((courseCode) => parsedCourseCodeSet.has(courseCode))
    );
    const categoryOptions = buildCategoryRequirementOptionsFromChoiceLine(
      owner,
      optionExtractionLine,
      parseRequirementCreditAmount(normalizedLine)
    );
...
function getLabNoteFallbackSubject(text, fallbackSubject = null) {
  const normalizedText = normalizeWhitespace(text);
  const explicitSubjects = getCourseCodeSubjects(extractCourseCodesFromLine(normalizedText));
  if (explicitSubjects.length === 1) {
    return explicitSubjects[0];
  }
  if (/\b(?:physics|phys)\b/i.test(normalizedText)) {
    return "PHYS";
  }
  if (/\b(?:chemistry|chem)\b/i.test(normalizedText)) {
    return "CHEM";
  }
  if (/\b(?:biology|biol)\b/i.test(normalizedText)) {
    return "BIOL";
  }
  if (/\b(?:mathematics|math|calculus)\b/i.test(normalizedText)) {
    return "MATH";
  }
...
function collectSequenceLabNotes(snapshotLines, startIndex, fallbackSubject) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (
      ...
      const continuationFallbackSubject = getLabNoteFallbackSubject(
        normalizeWhitespace([...labLines, continuationLine].join(" ")),
        fallbackSubject
      );
      const continuationLabCourses = extractCourseCodesFromSequenceText(
        continuationLine,
        continuationFallbackSubject
  ...
  const labFallbackSubject = getLabNoteFallbackSubject(labText, fallbackSubject);
  const conditionalLabCourses = extractCourseCodesFromSequenceText(labText, labFallbackSubject).filter(
...
function parseSectionHeadingCreditRange(line) {
  const normalizedLine = normalizeWhitespace(stripChoiceListLine(line));
  const leadingNamedBucketMatch = normalizedLine.match(
    /^(\d+(?:\.\d+)?)\s+(?:free\s+)?electives?\b/i
  );
  if (leadingNamedBucketMatch) {
    const credits = Number.parseFloat(leadingNamedBucketMatch[1]);
    if (Number.isFinite(credits) && credits > 0) {
      return { minCredits: credits, maxCredits: credits };
    }
  }
...
function getNearbySectionCourseTitle(snapshotLines, courseLineIndex) {
  for (
    ...
    if (
      ...
      /^(?:formal\s+options?|contact\b|advisor\b)/i.test(candidate) ||
      /\b(?:academic\s+advisor|track\s+your\s+progress|declare\s+(?:the|your)\s+major)\b/i.test(
        candidate
      )
    ) {
      return null;
...
function getInlineSectionCourseTitle(courseLine, primaryCourseCode) {
  const normalizedLine = normalizeWhitespace(stripChoiceListLine(courseLine));
  const courseNumber = normalizeCourseCode(primaryCourseCode).split(" ").at(-1);
  if (!normalizedLine || !courseNumber) {
    return null;
  }
  const numberMatch = new RegExp(`\\b${courseNumber}\\b`, "i").exec(normalizedLine);
  if (!numberMatch) {
    return null;
  }

  const afterCourseNumber = normalizedLine.slice(
    (numberMatch.index ?? 0) + numberMatch[0].length
  );
  const beforeAlternative = afterCourseNumber.split(/\s+or\s+(?=[A-Z])/i)[0];
  const nextCourseNumberMatch = /\b\d{3}[A-Za-z]?\b/.exec(beforeAlternative);
  const beforeNextCourse = nextCourseNumberMatch
    ? beforeAlternative
        .slice(0, nextCourseNumberMatch.index)
        .replace(/\s+(?:[A-Z&]{1,12}\s*){1,3}$/g, "")
    : beforeAlternative;
  const title = normalizeWhitespace(beforeNextCourse)
    .replace(/^\s*[:\-–—]\s*/, "")
    .replace(
      /\s*\(\s*\d+(?:\.\d+)?(?:\s*(?:-|to)\s*\d+(?:\.\d+)?)?\s*(?:credits?|cr\.?)?\s*\)\s*$/i,
      ""
    )
    .trim();
...
function buildSectionedOptionCourseOption(owner, input) {
  ...
  const title =
    getInlineSectionCourseTitle(input.courseLine, primaryCourseCode) ||
    metadata?.title ||
...
function buildStandaloneCourseChoiceRequirementGroups(owner, snapshotLines) {
  const groups = [];
  const seenGroupIds = new Set();

  for (let index = 0; index < (snapshotLines ?? []).length; index += 1) {
    const sourceRowText = normalizeWhitespace(stripChoiceListLine(snapshotLines[index]));
    if (
      !sourceRowText ||
      extractCourseCodesFromLine(sourceRowText).length > 0 ||
      !/\b(?:choose|select)\s+(?:one|two|three|four|five|six|\d+)(?:\s+courses?)?\s+(?:of|from)\s+(?:the\s+)?(?:following|list\s+below)\b/i.test(
        sourceRowText
      )
    ) {
      continue;
    ...
    const requiredCount = parseChoiceRequiredCount(sourceRowText);
    if (!Number.isFinite(requiredCount) || requiredCount < 1) {
      continue;
    }

    const category = buildSectionedCourseCategory(sourceRowText);
    const options = [];
    for (
      let optionIndex = index + 1;
      optionIndex < Math.min((snapshotLines ?? []).length, index + 42);
      optionIndex += 1
    ) {
      const optionLine = normalizeWhitespace(stripChoiceListLine(snapshotLines[optionIndex]));
      if (!optionLine) {
        continue;
      }

      const option = buildSectionedOptionCourseOption(owner, {
        courseLine: optionLine,
        title: getNearbySectionCourseTitle(snapshotLines, optionIndex),
        creditRange: parseRequirementCreditRange(optionLine),
        category,
        sectionHeading: sourceRowText,
        sourceHeading: sourceRowText,
      });
      if (option) {
        options.push(option);
        continue;
      ...
      if (
        options.length > 0 &&
        (isStandaloneSectionChoiceCueLine(optionLine) ||
          isSectionedOptionCourseBoundary(optionLine) ||
          looksLikeSectionedCourseHeadingLine(optionLine) ||
          isLikelyPeerPathwayHtmlSectionStartLine(optionLine))
      ) {
        break;
      }
    }

    const dedupedOptions = uniqueBy(options, (option) => option.id);
    if (dedupedOptions.length <= requiredCount) {
      continue;
    }

    const groupId = `${owner.ownerId}:requirement-group:${slugify(
      `${sourceRowText}-${requiredCount}-${dedupedOptions
    ...
    const group = buildParsedRequirementGroup({
      ...
      category,
      subcategory: category,
      requirementType: requiredCount === 1 ? "choose_one" : "choose_n",
      minCourses: requiredCount,
      maxCourses: requiredCount,
      selectionCount: requiredCount,
      requiredCount,
      sourceHeading: sourceRowText,
      sourceRowText,
      detectedOptionCue: "standalone course-count choice list",
      notes: ["Parsed from an explicit course-count choice cue and its following course rows."],
...
function isGraduateOrAppliedMastersRequirementContext(owner, snapshotLines = [], index = 0) {
  const ownerSourceUrl = owner?.sourceUrl ?? owner?.primarySourceUrl;
  const sourceContext = [
    owner?.sourceUrl,
  ...
  for (let lineIndex = priorContextLines.length - 1; lineIndex >= 0; lineIndex -= 1) {
    ...
    if (
      isUndergraduateCatalogRequirementResetLine(line) ||
      isUndergraduateCatalogSectionResetLine(line) ||
      hasUndergraduateRequirementResetCue(line) ||
      isTacomaRnBsnSampleProgramPlanResetLine(ownerSourceUrl, line)
    ) {
      break;
    }
    if (
...
function parseSectionedCourseRequiredCount(line) {
  ...
  if (/\b(?:choose|select)\s+(?:\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+courses?\b/i.test(normalizedLine)) {
    const explicitChoiceCount = parseChoiceRequiredCount(normalizedLine);
    if (Number.isFinite(explicitChoiceCount) && explicitChoiceCount > 0) {
      return explicitChoiceCount;
    }
  }
...
function looksLikeTitleAndCreditLineForPreviousCourse(previousLine, line) {
  const previousCourseCode = extractCourseCodesFromLine(previousLine)[0] ?? null;
  const courseTitle = normalizeWhitespace(getUwCourseMetadata(previousCourseCode)?.title ?? "");
  if (!courseTitle) {
    return false;
  }

  const lineWithoutTrailingCredits = normalizeWhitespace(line).replace(
    /\s*\(\s*\d+(?:\.\d+)?(?:\s*(?:-|to)\s*\d+(?:\.\d+)?)?\s*(?:credits?|cr\.?)\s*\)\s*$/i,
    ""
  );
  return normalizeMatcherText(lineWithoutTrailingCredits) === normalizeMatcherText(courseTitle);
}
...
function getSectionedCourseHeadingDescriptor(owner, snapshotLines, index, input = {}) {
  ...
  const looksLikeCourseDescriptionAfterCourseRow =
    ...
    (looksLikeTitleAndCreditLineForPreviousCourse(previousMeaningfulLine, rawLine) ||
      (rawLine.length >= 80 &&
        /[.!?]$/.test(rawLine) &&
        !hasPrimaryRequirementSectionCue(rawLine) &&
        !SECTIONED_COURSE_LIST_REQUIREMENT_PATTERN.test(rawLine)));
  const matchesExtractedHeading = (owner?.extractedHeadings ?? []).some(
    (heading) => normalizeWhitespace(heading) === rawLine
  );
  const hasStrongAdjacentSectionCue =
    matchesExtractedHeading ||
    SECTIONED_COURSE_LIST_REQUIREMENT_PATTERN.test(rawLine) ||
    /\b(?:requirements?|electives?|core courses?|capstone|concentration|track|option)\b/i.test(
      rawLine
    );
  const hasAdjacentCourseListEvidence =
    hasStrongAdjacentSectionCue ||
    hasUpcomingSectionedCourseRows(snapshotLines, index, 2);
  const looksLikeAdjacentCreditSectionHeading = Boolean(
    previousMeaningfulLine &&
      sourceLineStartsWithCourseCode(previousMeaningfulLine) &&
      parseSectionHeadingCreditRange(rawLine) &&
      looksLikeShortNamedCourseSectionHeading(rawLine) &&
      hasAdjacentCourseListEvidence &&
  ...
  if (
    ...
    (previousMeaningfulLine &&
      ...
      !looksLikeAdjacentCreditSectionHeading) ||
  ...
  const hasExplicitCourseCountChoiceCue =
    /\b(?:choose|select)\s+(?:\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+courses?\b/i.test(
      rawLine
    );
  ...
  if (requiredCount && hasExplicitCourseCountChoiceCue) {
    return {
      label,
      category,
      ...
      selectionCount: requiredCount,
      requiredCount,
      minCredits: exactCreditRange?.minCredits ?? null,
      maxCredits: exactCreditRange?.maxCredits ?? null,
      creditText: exactCreditRange
        ? exactCreditRange.maxCredits && exactCreditRange.maxCredits !== exactCreditRange.minCredits
          ? `${exactCreditRange.minCredits}-${exactCreditRange.maxCredits}`
          : String(exactCreditRange.minCredits)
        : null,
      detectedOptionCue: "sectioned course-count choice list",
    };
  }
...
function isSectionedCourseListBoundary(owner, snapshotLines, index, hasCollectedCourses, input = {}) {
  ...
  if (
    getSectionedCourseHeadingDescriptor(owner, snapshotLines, index, input) ||
    getFallbackSectionedElectiveHeadingDescriptor(owner, snapshotLines, index, input)
  ) {
    return true;
  }
  if (
    hasCollectedCourses &&
    !extractCourseCodesFromLine(line).length &&
    /\belectives?\b/i.test(line) &&
    parseSectionHeadingCreditRange(line) &&
    hasUpcomingSectionedCourseRows(snapshotLines, index, 2)
  ) {
    return true;
  }
...
function buildSourceDerivedSectionedCourseRequirementGroups(owner, snapshotLines) {
  ...
  for (let index = 0; index < (snapshotLines ?? []).length; index += 1) {
    ...
    const selectionCount = effectiveDescriptor.selectionCount ?? effectiveDescriptor.requiredCount;
    const fixedOptionCreditRanges = options
      .map((option) => {
        const minCredits = Number(option.creditMin ?? option.credits);
        const maxCredits = Number(option.creditMax ?? option.credits);
        return Number.isFinite(minCredits) && Number.isFinite(maxCredits)
          ? { minCredits, maxCredits }
          : null;
      });
    const selectionCreditRange =
      effectiveDescriptor.requirementType === "choose_n" &&
      Number.isInteger(selectionCount) &&
      selectionCount > 0 &&
      options.length >= selectionCount &&
      fixedOptionCreditRanges.every(Boolean)
        ? {
            minCredits: fixedOptionCreditRanges
              .map((range) => range.minCredits)
              .sort((left, right) => left - right)
              .slice(0, selectionCount)
              .reduce((sum, credits) => sum + credits, 0),
            maxCredits: fixedOptionCreditRanges
              .map((range) => range.maxCredits)
              .sort((left, right) => right - left)
              .slice(0, selectionCount)
              .reduce((sum, credits) => sum + credits, 0),
          }
        : null;
    const groupDescriptor = selectionCreditRange
      ? {
          ...effectiveDescriptor,
          minCredits: selectionCreditRange.minCredits,
          maxCredits: selectionCreditRange.maxCredits,
          creditText:
            selectionCreditRange.minCredits === selectionCreditRange.maxCredits
              ? String(selectionCreditRange.minCredits)
              : `${selectionCreditRange.minCredits}-${selectionCreditRange.maxCredits}`,
        }
      : effectiveDescriptor;
    const descriptorMinimum =
      groupDescriptor.requirementType === "choose_n"
        ? groupDescriptor.minCourses
        : groupDescriptor.minCredits;
    const descriptorMaximum =
      groupDescriptor.requirementType === "choose_n"
        ? groupDescriptor.maxCourses
        : groupDescriptor.maxCredits;

    const groupId = `${owner.ownerId}:requirement-group:${slugify(
      `${groupDescriptor.label}-${groupDescriptor.requirementType}-${descriptorMinimum ?? "options"}-${descriptorMaximum ?? "open"}`
    )}`;
    if (seenGroupIds.has(groupId)) {
      continue;
    }
    ...
    const group = buildParsedRequirementGroup({
      ...
      category: groupDescriptor.category,
      subcategory: groupDescriptor.category,
      requirementType: groupDescriptor.requirementType,
      minCourses: groupDescriptor.minCourses,
      maxCourses: groupDescriptor.maxCourses,
      selectionCount: groupDescriptor.selectionCount,
      requiredCount: groupDescriptor.requiredCount,
      minCredits: groupDescriptor.minCredits,
      maxCredits: groupDescriptor.maxCredits,
      creditText: groupDescriptor.creditText,
      sourceHeading,
      sourceRowText: sourceHeading,
      detectedOptionCue: groupDescriptor.detectedOptionCue,
      notes: [
...
function extractInlineRequiredListAlternativeSets(text) {
  ...
  const parentheticalAlternativePattern =
    /\b([A-Za-z&]{1,20}(?:\s+[A-Za-z&]{1,20})?)\s*(\d{3}[A-Za-z]?)\s*\(\s*or\s+([^)]*)\)/gi;

  for (const match of normalizedText.matchAll(parentheticalAlternativePattern)) {
    const leadingCourseCode = normalizeExtractedCourseCode(match[1], match[2]);
    const courseCodes = uniqueInOrder(
      [
        leadingCourseCode,
        ...extractCourseCodesFromLine(`${match[1]} ${match[2]} or ${match[3]}`),
      ]
        .filter(Boolean)
        .map(normalizeCourseCode)
    );
  ...
  const uniqueAlternativeSets = uniqueBy(alternativeSets, (courseCodes) =>
    courseCodes.join("|")
  );
  return uniqueAlternativeSets.filter(
    (courseCodes, index) =>
      !uniqueAlternativeSets.some(
        (candidate, candidateIndex) =>
          candidateIndex !== index &&
          candidate.length > courseCodes.length &&
          courseCodes.every((courseCode) => candidate.includes(courseCode))
      )
  );
}

function getRepeatedRequiredAlternativeSets(text) {
  const normalizedText = normalizeWhitespace(text);
  const inlineParts = getInlineLabeledRequirementParts(normalizedText);
  if (
    inlineParts ||
    /\b(?:choose|select|one\s+of|any\s+of|either)\b/i.test(normalizedText) ||
    (!/\band\b/i.test(normalizedText) && !/,\s*[^,]{0,80}\bor\b/i.test(normalizedText))
  ) {
    return [];
  }

  const alternativeSets = extractInlineRequiredListAlternativeSets(normalizedText);
  if (alternativeSets.length < 2) {
    return [];
  }

  const flattenedCourseCodes = alternativeSets.flat();
  if (new Set(flattenedCourseCodes).size !== flattenedCourseCodes.length) {
    return [];
  }
...
function buildRepeatedRequiredAlternativeGroups(
  owner,
  sourceLine,
  optionExtractionLine,
  sourceSection
) {
  return getRepeatedRequiredAlternativeSets(optionExtractionLine).map(
    (alternativeSet, index) => {
      const groupId = `${owner.ownerId}:requirement-group:required-alternative-slot-${
        index + 1
      }-${slugify(alternativeSet.join("-or-"))}`;
      return buildParsedRequirementGroup({
        id: groupId,
        label: alternativeSet.join(" / "),
        category: "course-alias",
        subcategory: "required-alternative-slot",
        requirementType: "all_required",
        minCourses: 1,
        maxCourses: 1,
        sourceHeading: sourceLine,
        ...
        sourceSection,
        detectedOptionCue: "repeated required alternative slots",
        notes: [
          "Each repeated source slot is required; the courses inside this slot are alternatives.",
        ],
        options: [
          {
            id: `${groupId}:option:${slugify(alternativeSet.join("-or-"))}`,
            uwCourses: [alternativeSet[0]],
            equivalentUwCourseCodes: alternativeSet.slice(1),
            label: alternativeSet.join(" / "),
          },
        ],
      });
...
function getInlineRequirementSectionClassification(owner, snapshotLines, targetIndex) {
  ...
  for (let index = 0; index <= targetIndex && index < (snapshotLines ?? []).length; index += 1) {
    ...
    const explicit = isGraduateOrAppliedMastersRequirementContext(owner, snapshotLines, index)
      ...
      : inDegreeRouteComparisonSection
        ...
        : classifySourceSectionRoleForLine(normalizedLine, currentRole, currentRoleReason);
  ...
  return {
    ...
    reason: currentRoleReason ?? "inherits nearby source-section role",
  };
}
...
function buildGenericChoiceRequirementGroups(owner, parsedCourseCodes, snapshotLines) {
  ...
  for (const choiceSourceLine of choiceSourceLines) {
    ...
    const optionExtractionLine = stripNonOptionCourseEvidenceFromChoiceLine(normalizedLine);
    if (
      hasCourseQuestionSupportCue(optionExtractionLine) ||
      hasNonRequirementCourseExclusionCue(optionExtractionLine)
    ) {
      continue;
    }
    const repeatedRequiredAlternativeGroups = buildRepeatedRequiredAlternativeGroups(
      owner,
      normalizedLine,
      optionExtractionLine,
      choiceSourceLine.nearbyRequirementLabel
    );
    if (repeatedRequiredAlternativeGroups.length) {
      for (const group of repeatedRequiredAlternativeGroups) {
    ...
    if (!categoryOptions.length && isEquivalentNumberAliasLine(optionExtractionLine, courseCodes)) {
      ...
      group = buildParsedRequirementGroup({
        ...
        selectionCount: requiredCount,
        requiredCount,
        minCredits: Number.isFinite(credits) ? credits : null,
        maxCredits: Number.isFinite(credits) ? credits : null,
        creditText: Number.isFinite(credits) ? String(credits) : null,
        options: [
          ...courseCodes.map((courseCode) => ({
            id: `${owner.ownerId}:requirement-option:${slugify(courseCode)}`,
            uwCourses: [courseCode],
            label: courseCode,
            ...(getUwCourseCreditRange(courseCode) ?? {}),
          })),
          ...categoryOptions,
        ],
      });
...
function suppressChoiceGroupsCoveredByCreditBuckets(groups) {
  ...
  const creditBucketGroupsBySource = creditBucketGroups
    .map((group) => {
      const sourceKey = normalizeMatcherText(
        group.sourceRowText ?? group.sourceHeading ?? group.label ?? ""
      );
      ...
      return { sourceKey, codeSet };
    })
    .filter((entry) => entry.sourceKey && entry.codeSet.size > 0);
  ...
  return (groups ?? []).filter((group) => {
    if (
      ...
      /inline required course list with embedded alternatives/i.test(
        group.detectedOptionCue ?? ""
      )
    ) {
      const groupSourceKey = normalizeMatcherText(
        group.sourceRowText ?? group.sourceHeading ?? group.label ?? ""
      );
      ...
      if (
        groupSourceKey &&
        groupCodeSet.size > 0 &&
        creditBucketGroupsBySource.some(
          (bucket) =>
            bucket.sourceKey === groupSourceKey &&
            [...groupCodeSet].every((courseCode) => bucket.codeSet.has(courseCode))
        )
      ) {
        return false;
      }
...
function suppressRepeatedRequiredAlternativeGroupsCoveredBySequenceChoices(groups) {
  const sequencePathSourceTexts = new Set(
    (groups ?? [])
      .filter((group) => group?.requirementType === "sequence_choice")
      .flatMap((group) => group.sequencePaths ?? [])
      .map((path) => normalizeMatcherText(path?.sourceText ?? ""))
      .filter(Boolean)
  );
  if (!sequencePathSourceTexts.size) {
    return groups;
  }

  return (groups ?? []).filter((group) => {
    if (group?.subcategory !== "required-alternative-slot") {
      return true;
    }
    const sourceText = normalizeMatcherText(
      group?.sourceRowText ?? group?.sourceHeading ?? ""
    );
    return !sourceText || !sequencePathSourceTexts.has(sourceText);
  });
}
...
function getChoiceRequirementGroupCourseSet(group) {
  const key = getChoiceRequirementGroupCourseSetKey(group);
  return new Set(key ? key.split("|") : []);
}

function isSourceChoiceRequirementGroup(group) {
  return (
    group?.category === "source-choice" &&
    ["choose_one", "choose_n"].includes(group?.requirementType)
  );
}

function getSourceChoiceRequirementGroupScopeKey(group) {
  if (!isSourceChoiceRequirementGroup(group)) {
    return "";
  }
  const labelKey = normalizeMatcherText(group?.label ?? "");
  const sourceKey = normalizeMatcherText(
    group?.sourceRowText ?? group?.sourceHeading ?? ""
  );
  ...
  const requiredCount =
    ...
    (group?.minCourses != null && group.minCourses === group?.maxCourses
      ? group.minCourses
      : "");
  return `${group.requirementType}:${requiredCount}:${labelKey}:${sourceKey}`;
}

function isProperCourseSetSubset(candidateSubset, candidateSuperset) {
  return (
    candidateSubset.size > 0 &&
    candidateSuperset.size > candidateSubset.size &&
    [...candidateSubset].every((courseCode) => candidateSuperset.has(courseCode))
  );
}

function suppressSubsetSourceChoiceGroups(groups) {
  const sourceChoiceGroups = (groups ?? [])
    .map((group, index) => ({
      group,
      index,
      scopeKey: getSourceChoiceRequirementGroupScopeKey(group),
      courseSet: getChoiceRequirementGroupCourseSet(group),
    }))
    .filter((entry) => entry.scopeKey && entry.courseSet.size > 0);

  if (sourceChoiceGroups.length < 2) {
    return groups;
  }

  const suppressedIndexes = new Set();
  for (const entry of sourceChoiceGroups) {
    const hasSameSourceSuperset = sourceChoiceGroups.some(
      (candidate) =>
        candidate.index !== entry.index &&
        candidate.scopeKey === entry.scopeKey &&
        isProperCourseSetSubset(entry.courseSet, candidate.courseSet)
    );
    if (hasSameSourceSuperset) {
      suppressedIndexes.add(entry.index);
    }
  }

  return (groups ?? []).filter((_, index) => !suppressedIndexes.has(index));
}
...
function coalesceSectionedCreditBucketsWithInlineChoices(owner, groups) {
  const replacementsByIndex = new Map();
  const consumedIndexes = new Set();

  for (let bucketIndex = 0; bucketIndex < (groups ?? []).length; bucketIndex += 1) {
    const bucket = groups[bucketIndex];
    if (
      bucket?.requirementType !== "choose_credits" ||
      bucket?.detectedOptionCue !== "sectioned credit list"
    ) {
      continue;
    }

    const bucketCourseCodes = uniqueInOrder(
      (bucket.options ?? [])
        .flatMap((option) => option.uwCourses ?? [])
        .map((courseCode) => normalizeCourseCode(courseCode))
        .filter(Boolean)
    );
    if (bucketCourseCodes.length < 2) {
      continue;
    }

    const choiceIndex = (groups ?? []).findIndex((candidate, candidateIndex) => {
      if (
        candidateIndex === bucketIndex ||
        consumedIndexes.has(candidateIndex) ||
        candidate?.requirementType !== "choose_one" ||
        candidate?.category !== "source-choice" ||
        !/\bor\b/i.test(candidate?.sourceRowText ?? candidate?.sourceHeading ?? "")
      ) {
        return false;
      }
      const choiceCourseCodes = uniqueInOrder(
        (candidate.options ?? [])
          .flatMap((option) => option.uwCourses ?? [])
          .map((courseCode) => normalizeCourseCode(courseCode))
          .filter(Boolean)
      ...
      return (
        choiceCourseCodes.length >= 2 &&
        choiceCourseCodes.some((courseCode) => bucketCourseCodes.includes(courseCode))
      );
    });
    if (choiceIndex < 0) {
      continue;
    ...
    const choice = groups[choiceIndex];
    const choiceCourseCodes = uniqueInOrder(
      (choice.options ?? [])
        .flatMap((option) => option.uwCourses ?? [])
        .map((courseCode) => normalizeCourseCode(courseCode))
    ...
    const requiredCourseCodes = bucketCourseCodes.filter(
      (courseCode) => !choiceCourseCodes.includes(courseCode)
    );
    if (!requiredCourseCodes.length) {
      continue;
    }

    const sequenceGroup = buildSequenceChoiceRequirementGroup(owner, {
      rawText: normalizeWhitespace(bucket.sourceHeading ?? bucket.label),
      sourceSection: bucket.sourceSection,
      detectedOptionCue: "sectioned credit list with inline course alternative",
      paths: choiceCourseCodes.map((choiceCourseCode) => ({
        label: [...requiredCourseCodes, choiceCourseCode].join(" + "),
        uwCourses: [...requiredCourseCodes, choiceCourseCode],
        sourceText: normalizeWhitespace(choice.sourceRowText ?? choice.sourceHeading),
      })),
    });
    if (!sequenceGroup) {
      continue;
    ...
    replacementsByIndex.set(bucketIndex, {
      ...sequenceGroup,
      label: bucket.label,
      category: bucket.category,
      subcategory: bucket.subcategory,
      minCredits: bucket.minCredits,
      maxCredits: bucket.maxCredits,
      creditText: bucket.creditText,
      sourceHeading: bucket.sourceHeading,
      sourceRowText: bucket.sourceRowText,
      notes: uniqueSorted([
        ...(bucket.notes ?? []),
        "Combined required section courses with the source's inline course alternative.",
      ]),
    });
  ...
  return (groups ?? []).flatMap((group, index) => {
    ...
    return [replacementsByIndex.get(index) ?? group];
  });
}
    ...
    suppressOwnerSpecificRequirementGroupLeaks(
      ...
      dedupeCreditBucketRequirementGroups(
        suppressChoiceGroupsCoveredByCreditBuckets(
          suppressSubsetSourceChoiceGroups(
            suppressGenericChoiceGroupsCoveredByInlineChoices(
              suppressRepeatedRequiredAlternativeGroupsCoveredBySequenceChoices(
                suppressInlineRequiredGroupsCoveredByChoiceOptions(
                  coalesceSectionedCreditBucketsWithInlineChoices(
                    owner,
                    filterRequirementGroupsBySupplementalPathwayScope(
                      ...
                      [
                        ...
                        ...buildStandaloneCourseChoiceRequirementGroups(owner, snapshotLines),
                        ...
                        ...buildForwardLabeledChoiceRequirementGroups(owner, parsedCourseCodes, snapshotLines),
...
function lineMatchesPathwayHeadingIdentity(headingLine, candidateLine) {
  ...
  const requiredMatches = tokens.length <= 2 ? tokens.length : 2;
  if (tokenMatches >= Math.max(1, requiredMatches)) {
    return true;
  }

  const headingAcronyms = new Set(
    (stripPathwayHeadingPrefix(headingLine).match(/\b[A-Z]{2,6}\b/g) ?? []).filter(
      (token) => !/^(?:BA|BS|BABA)$/i.test(token)
    )
  );
  const candidateAcronyms =
    stripPathwayHeadingPrefix(candidateLine).match(/\b[A-Z]{2,6}\b/g) ?? [];
  return candidateAcronyms.some((token) => headingAcronyms.has(token));
}
...
function parsedMentionsSelectedPathwayIdentity(entry, parsed) {
  return (parsed?.snapshotLines ?? []).some((line) =>
    lineMatchesSelectedPathwayIdentity(entry, line)
  );
}

function parsedMentionsSiblingPathwaySection(entry, parsed) {
  return (parsed?.snapshotLines ?? []).some((line) =>
    isPathwayHtmlSectionSiblingStart(entry, line)
  );
}
...
function findPriorAdmitCurriculumStartIndex(lines) {
  if (!Array.isArray(lines) || lines.length < 12) {
    return null;
  }

  for (let index = 0; index < lines.length; index += 1) {
    if (!PRIOR_ADMIT_CURRICULUM_START_PATTERN.test(normalizeWhitespace(lines[index]))) {
      continue;
    }

    const followingLines = lines.slice(index, Math.min(lines.length, index + 36));
    const followingText = normalizeMatcherText(followingLines.join(" "));
    const followingCourseCount = uniqueSorted(
      followingLines.flatMap((line) => extractCourseCodesFromLine(line))
    ).length;
    if (
      followingCourseCount >= 3 &&
      /\bmajor requirements?\b/.test(followingText) &&
      /\b(?:introductory|methods?|formal options?)\b/.test(followingText)
    ) {
      return index;
    }
  }
...
function isPriorAdmitCurriculumOwner(entry) {
  return getPriorAdmitCurriculumOwnerEra(entry) !== null;
}

function scopeCurriculumEraLines(entry, lines) {
  const legacyStartIndex = findPriorAdmitCurriculumStartIndex(lines);
  if (legacyStartIndex === null) {
    return lines;
  }

  if (!isPriorAdmitCurriculumOwner(entry)) {
    return removePriorAdmitCurriculumSectionsFromCurrentScope(lines);
  }

  let legacyEndIndex = lines.length - 1;
  for (let index = legacyStartIndex + 1; index < lines.length; index += 1) {
    const line = normalizeWhitespace(lines[index]);
    if (/^(?:graduation requirements?|contact)$/i.test(line)) {
      legacyEndIndex = Math.max(legacyStartIndex, index - 1);
      break;
    }
  }

  return lines.slice(legacyStartIndex, legacyEndIndex + 1);
}
...
function buildParallelPathwayHtmlTableScope(entry, lines, peerStartIndexes) {
  ...
  for (const selectedCandidate of peerStartIndexes.filter((candidate) => candidate.selected)) {
    ...
    while (
      peerIndexByLineIndex.has(runStart - 1) ||
      isParallelPathwayHeadingBridgeLine(lines[runStart - 1])
    ) {
      runStart -= 1;
    }
    ...
    while (
      peerIndexByLineIndex.has(runEnd + 1) ||
      isParallelPathwayHeadingBridgeLine(lines[runEnd + 1])
    ) {
      runEnd += 1;
    }
    ...
    const endIndex =
      nextSiblingContentIndex !== null && nextSiblingContentIndex > startIndex
        ...
        : defaultEndIndex;
    const selectedHeadingBridgeLines =
      selectedPosition < runIndexes.length - 1
        ? lines
            .slice(selectedCandidate.index + 1, runIndexes[selectedPosition + 1])
            .filter(isParallelPathwayHeadingBridgeLine)
        : [];
    const sectionLines = uniqueInOrder([
      selectedHeading,
      ...selectedHeadingBridgeLines,
...
function isParallelPathwayHeadingBridgeLine(line) {
  const normalizedLine = normalizeWhitespace(stripChoiceListLine(line));
  return (
    Boolean(normalizedLine) &&
    extractCourseCodesFromLine(normalizedLine).length === 0 &&
    normalizedLine.split(/\s+/).length <= 14 &&
    /^(?:choose|complete|select)\b.*\b(?:courses?|credits?|following|below)\b/i.test(
      normalizedLine
    )
  );
}
...
function getPathwayHtmlSectionScopeLines(entry, lines, pathwaySectionRange) {
  const selectedLines =
    pathwaySectionRange.lines ??
    lines.slice(pathwaySectionRange.startIndex, pathwaySectionRange.endIndex + 1);
  if (!isPriorAdmitCurriculumOwner(entry)) {
    return selectedLines;
  }

  const formalOptionsIndex = lines.findIndex((line) =>
    /^formal options?\b/i.test(normalizeWhitespace(stripChoiceListLine(line)))
  );
  if (formalOptionsIndex < 0) {
    return selectedLines;
  }

  const firstPathwayHeadingIndex = lines.findIndex(
    (line, index) =>
      index > formalOptionsIndex && isLikelyPeerPathwayHtmlSectionStartLine(line)
  );
  if (firstPathwayHeadingIndex < 0) {
    return selectedLines;
  }

  return uniqueInOrder([
    ...lines.slice(0, firstPathwayHeadingIndex),
    ...selectedLines,
  ]);
}
...
function shouldUseFullAdmissionPrerequisiteHtmlScope(entry, sourceLines) {
  const entryText = normalizeMatcherText(
    `${entry?.parserType ?? ""} ${entry?.role ?? ""} ${entry?.label ?? ""} ${entry?.url ?? ""}`
  );
  if (!/\b(?:admissions?|prereq(?:uisites?)?)\b/.test(entryText)) {
    return false;
  }

  const parsedCourseCount = uniqueSorted(
    (sourceLines ?? []).flatMap((line) => extractCourseCodesFromLine(line))
  ).length;
  if (parsedCourseCount < 3) {
    return false;
  }

  const sourceText = normalizeMatcherText((sourceLines ?? []).join(" "));
  return /\b(?:admission requirements?|application eligibility|prereq(?:uisite)? courses?|business prereq(?:uisite)?s?)\b/.test(
    sourceText
  );
}

function scopeHtmlLines(entry, title, headings, lines) {
  const currentCatalogLines = scopeCurriculumEraLines(entry, lines);
  ...
  if (shouldUseFullAdmissionPrerequisiteHtmlScope(entry, sourceLines)) {
    return sourceLines;
  }

  const pathwaySectionRange = findPathwayHtmlSectionRange(entry, sourceLines);
  if (pathwaySectionRange) {
    return getPathwayHtmlSectionScopeLines(entry, sourceLines, pathwaySectionRange);
  }
...
function collapseHtmlTableRows(html) {
  return String(html ?? "").replace(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi, (rowHtml, rowBody) => {
    const cells = [...rowBody.matchAll(/<(td|th)\b[^>]*>([\s\S]*?)<\/\1>/gi)]
      .map((match) => stripHtml(match[2]))
      .filter(Boolean);
    const hasSplitSubjectAndNumber =
      cells.length >= 2 &&
      /^[A-Za-z&]{1,20}(?:\s+[A-Za-z&]{1,20})?$/.test(cells[0]) &&
      /^\d{3}[A-Za-z]?\b/.test(cells[1]);
    return hasSplitSubjectAndNumber ? `\n${cells.join(" ")}\n` : rowHtml;
  });
}

function buildHtmlLines(html) {
  return collapseHtmlTableRows(stripHtmlDocumentChrome(html))
...
function isDedicatedSelectedPathwayHtmlSource(entry, fullParsed) {
  if (!entry?.pathwayId || isLegacyStudentCatalogSource(entry)) {
    return false;
  }

  const sourceIdentity = normalizeMatcherText(
    [
      entry.url,
      entry.label,
      entry.sourceLabel,
  ...
  const tokenGroups = getStrictSelectedPathwayScopeTokenGroups(entry);
  return tokenGroups.some(
    (tokens) => tokens.length > 0 && tokens.every((token) => sourceIdentity.includes(token))
  );
}

function isDedicatedSelectedPathwayRouteSource(entry, fullParsed) {
  if (!entry?.pathwayId || isLegacyStudentCatalogSource(entry)) {
    return false;
  }

  const pathwayDegreeKind = getBaBsDegreeKind(
    `${entry.pathwayId ?? ""} ${entry.ownerTitle ?? ""}`
  );
  if (!pathwayDegreeKind) {
    return false;
  }

  const sourceDegreeKind = getBaBsDegreeKind(
    [
      entry.url,
      entry.label,
      entry.sourceLabel,
  ...
  return sourceDegreeKind === pathwayDegreeKind;
}
...
function selectPreferredHtmlParsed(entry, fullParsed, scopedParsed) {
  ...
  const dedicatedSelectedPathwaySource = isDedicatedSelectedPathwayHtmlSource(
    entry,
    fullParsed
  ) && (
    !parsedMentionsSiblingPathwaySection(entry, fullParsed) ||
    isDedicatedSelectedPathwayRouteSource(entry, fullParsed)
  );
  ...
  if (
    dedicatedSelectedPathwaySource &&
    fullCourseCount > scopedCourseCount &&
    fullCourseCount <= Math.max(scopedCourseCount + 80, scopedCourseCount * 4) &&
  ...
  if (
    entry?.pathwayId &&
    scopedCourseCount >= 1 &&
    parsedMentionsSelectedPathwayIdentity(entry, scopedParsed) &&
    parsedMentionsSiblingPathwaySection(entry, fullParsed)
  ) {
    return enrichedScopedParsed;
  }
...
async function parseHtmlSource(entry, timeoutMs, options = {}) {
  ...
  if (preferredDocumentSource) {
    return mergeParsedSources(
      mergedHtmlParsed,
      [preferredDocumentSource],
      preferredDocumentSource.parsed?.resolvedParserType ?? entry.parserType
    );
  }

  const titleOnlyBackfillDocumentSources = buildTitleOnlyCourseBackfillSupplementalSources(
    mergedHtmlParsed,
    linkedDocumentSources
  );
  if (titleOnlyBackfillDocumentSources.length) {
    return mergeParsedSources(
      mergedHtmlParsed,
      titleOnlyBackfillDocumentSources,
      entry.parserType
    );
  }
...
function getActiveManifestDocumentSourceUrlSetForOwner(entry) {
  return new Set(
    [...getActiveParserRecoverySourceUrlSetForOwner(entry)].filter((url) =>
      isLinkedDocumentSourceUrl(url)
    )
  );
}

function shouldKeepLinkedSupplementalDocumentForOwner(entry, url) {
  const activeDocumentUrls = getActiveManifestDocumentSourceUrlSetForOwner(entry);
  if (!activeDocumentUrls.size) {
    return true;
  }

  const normalizedUrl = normalizeUrlForComparison(url);
  return Boolean(normalizedUrl && activeDocumentUrls.has(normalizedUrl));
}
...
function extractSupplementalDocumentLinkCandidates(entry, html) {
  ...
  for (const match of String(html ?? "").matchAll(HTML_LINK_PATTERN)) {
    ...
    if (!shouldKeepLinkedSupplementalDocumentForOwner(entry, resolvedUrl)) {
      continue;
    }
...
function baseParsedMentionsTitleOnlyCalculusOrdinal(baseParsed, ordinal) {
  const text = normalizeWhitespace(
    [
      ...(baseParsed?.snapshotLines ?? []),
      ...(baseParsed?.requirementCueLines ?? []),
    ].join(" ")
  );
  if (!text) {
  ...
  return new RegExp(
    `\\bcalculus\\s*(?:${ordinal.value}|${ordinal.roman})\\b`,
    "i"
  ).test(text);
}

function lineHasCourseCodeWithCalculusOrdinal(line, courseCode, ordinal) {
  const normalizedLine = normalizeWhitespace(line);
  const normalizedCourseCode = normalizeCourseCode(courseCode);
  ...
  const courseCodePattern = new RegExp(
    `\\b${escapeRegex(normalizedCourseCode).replace(/\\ /g, "\\s+")}\\b`,
    "i"
  );
  const match = normalizedLine.match(courseCodePattern);
  if (!match || match.index == null) {
    return false;
  }

  const afterCourseCode = normalizedLine.slice(
    match.index + match[0].length,
    match.index + match[0].length + 80
  );
  return new RegExp(
    `^\\s*(?!pre[-\\s]?calculus\\b)calculus\\s*(?:${ordinal.value}|${ordinal.roman})\\b`,
    "i"
  ).test(afterCourseCode);
}

function lineContainsCourseCode(line, courseCode) {
  const normalizedLine = normalizeWhitespace(line);
  const normalizedCourseCode = normalizeCourseCode(courseCode);
  if (!normalizedLine || !normalizedCourseCode) {
    return false;
  ...
  return courseCodePattern.test(normalizedLine);
}

function lineMentionsCalculusOrdinalTitle(line, ordinal) {
  const textWithoutPrecalculus = normalizeWhitespace(line).replace(
    /\bpre[-\s]?calculus\s*(?:1|2|3|4|i|ii|iii|iv)\b/gi,
    " "
  );
  return new RegExp(
    `\\bcalculus\\s*(?:${ordinal.value}|${ordinal.roman})\\b`,
    "i"
  ).test(textWithoutPrecalculus);
}

function courseCodeLooksLikeCalculusOrdinalCode(courseCode, ordinal) {
  const match = normalizeCourseCode(courseCode).match(/\b(\d{3})[A-Z]?$/);
  const expectedCourseNumber = new Map([
    [1, "124"],
    [2, "125"],
    [3, "126"],
  ]).get(ordinal.value);
  return Boolean(match?.[1] && expectedCourseNumber && match[1] === expectedCourseNumber);
}

function findDocumentLineForCourseCodeWithCalculusOrdinal(snapshotLines, courseCode, ordinal) {
  for (let index = 0; index < snapshotLines.length; index += 1) {
    const line = snapshotLines[index];
    if (!lineContainsCourseCode(line, courseCode)) {
      continue;
    }
    if (lineHasCourseCodeWithCalculusOrdinal(line, courseCode, ordinal)) {
      return line;
    }

    if (!courseCodeLooksLikeCalculusOrdinalCode(courseCode, ordinal)) {
      continue;
    }

    const nearbyTitleLine = snapshotLines
      .slice(Math.max(0, index - 6), Math.min(snapshotLines.length, index + 7))
      .find((candidateLine) => lineMentionsCalculusOrdinalTitle(candidateLine, ordinal));
    if (nearbyTitleLine) {
      return `${normalizeCourseCode(courseCode)} Calculus ${ordinal.roman.toUpperCase()}`;
    }
  }
...
function buildTitleOnlyCourseBackfillSupplementalSources(baseParsed, linkedDocumentSources) {
  const recoveredSources = [];
  const existingCourseCodes = new Set(
    (baseParsed?.courseCodes ?? []).map((courseCode) => normalizeCourseCode(courseCode))
  );
  const neededCalculusOrdinals = CALCULUS_TITLE_ORDINALS.filter((ordinal) =>
    baseParsedMentionsTitleOnlyCalculusOrdinal(baseParsed, ordinal)
  );
  if (!neededCalculusOrdinals.length) {
    return recoveredSources;
  }

  for (const supplementalSource of linkedDocumentSources ?? []) {
    const { candidate, parsed } = supplementalSource ?? {};
    if (
      !parsed ||
    ...
    const snapshotLines = parsed.snapshotLines ?? [];
    const recoveredCourseCodes = [];
    const recoveredSnapshotLines = [];

    const candidateCourseCodes = uniqueSorted([
      ...(parsed.courseCodes ?? []),
      ...snapshotLines.flatMap((line) => extractCourseCodesFromLine(line)),
    ]);

    for (const courseCode of candidateCourseCodes) {
      const normalizedCourseCode = normalizeCourseCode(courseCode);
      if (!normalizedCourseCode || existingCourseCodes.has(normalizedCourseCode)) {
        continue;
      }

      const matchingLine = neededCalculusOrdinals
        .map((ordinal) =>
          findDocumentLineForCourseCodeWithCalculusOrdinal(
            snapshotLines,
            normalizedCourseCode,
            ordinal
          )
      ...
      recoveredCourseCodes.push(normalizedCourseCode);
      recoveredSnapshotLines.push(matchingLine);
      existingCourseCodes.add(normalizedCourseCode);
    }

    if (!recoveredCourseCodes.length) {
      continue;
    }

    const normalizedRecoveredSnapshotLines = uniqueSorted(recoveredSnapshotLines);
    recoveredSources.push({
      ...supplementalSource,
      parsed: {
        ...parsed,
        headings: [],
        requirementCueLines: ["Prerequisites", ...normalizedRecoveredSnapshotLines],
        chooseStatements: [],
        pathwayLabels: [],
        courseCodes: uniqueSorted(recoveredCourseCodes),
        snapshotLines: ["Prerequisites", ...normalizedRecoveredSnapshotLines],
        parseConfidence: buildParseConfidence(
          recoveredCourseCodes,
          recoveredSnapshotLines,
          candidate?.parserType ?? parsed.resolvedParserType ?? "pdf-degree-sheet"
        ),
      },
    });
  }
...
function parsedDocumentHasTitleOnlyBackfillEvidence(parsed) {
  const snapshotLines = parsed?.snapshotLines ?? [];
  return snapshotLines.some((line) =>
    extractCourseCodesFromLine(line).some((courseCode) =>
      CALCULUS_TITLE_ORDINALS.some((ordinal) =>
        findDocumentLineForCourseCodeWithCalculusOrdinal(snapshotLines, courseCode, ordinal)
      )
    )
  );
}
...
async function parseSupplementalDocumentSources(entry, html, timeoutMs, visitedUrls) {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      const canUseForTitleOnlyBackfill =
        candidate.titleAcronymMatch && parsedDocumentHasTitleOnlyBackfillEvidence(parsed);
      if (parsedAlignmentScore < 2 && !canUseForTitleOnlyBackfill) {
        continue;
      }
...
function shouldAllowAlternateToReplaceBestSource(
  ...
  if (parserRecoveryCandidateConflictsWithPathway(bestEntry, alternateEntry?.label, alternateEntry?.url)) {
    return false;
  }
...
function shouldMergeSupplementalAlternateSource(
  ...
  if (parserRecoveryCandidateConflictsWithPathway(baseEntry, alternateEntry?.label, alternateEntry?.url)) {
    return false;
  }
...
function parserRecoveryCandidateConflictsWithOwnerIdentity(entry, label, url) {
  ...
  const sourceDegreeKind = getBaBsDegreeKind(
    `${entry?.pathwayId ?? ""} ${entry?.sourceLabel ?? ""} ${entry?.label ?? ""} ${entry?.url ?? ""}`
  );
...
function buildParserRecoverySectionCandidates(entry, artifacts) {
  ...
  const lines = removeFollowingDifferentOwnerAcronymDepartmentalSections(
    entry,
    scopeCurriculumEraLines(entry, rawLines)
  );
  ...
  if (pathwaySectionRange) {
    const sectionLines = getPathwayHtmlSectionScopeLines(entry, lines, pathwaySectionRange);
...
function getActiveParserRecoverySourceUrlSetForOwner(entry) {
  ...
  for (const candidate of TRANSFER_PLANNER_MANIFEST_REGISTRY) {
    if (
      candidate.ownerId !== entry.ownerId ||
      candidate.campusId !== entry.campusId ||
      !PARSEABLE_PARSER_TYPES.has(candidate.parserType) ||
...
function parserRecoverySnapshotIsActiveForOwner(entry, snapshot) {
...
function buildParserRecoveryOwnerSnapshotCandidates(entry, currentArtifacts = null) {
  ...
  for (const snapshot of readOwnerSnapshots(entry.ownerId)) {
    ...
    if (!parserRecoverySnapshotIsActiveForOwner(entry, snapshot)) {
      continue;
    }
...
function getRequirementSupportListSemanticKey(supportList) {
  ...
  if (
    ...
    return `approved:${shape}:${approvedListKey}`;
  }
...
function buildRequirementSupportListsFromMetadata(input) {
  ...
  const remainingSupportOnlyCodes =
    sourceRole === "approved-course-list" && approvedCodes.length
      ? []
      : supportOnlyCodes.filter((courseCode) => !usedCodes.has(courseCode));
...
function buildManifestParseSuccess(
  ...
  const sourceRole = sourceInactiveMajor
    ? "non-schedulable-course-list"
    : shouldPromotePrimaryOverviewFromParsedEvidence(
        effectiveEntry,
        parsedSourceRole,
        metadataSourceRole,
        parsed
  ...
  const explicitlyExcludedCourseCodeSet = new Set(
    getGloballyExplicitlyExcludedCourseCodes(
      parsed.snapshotLines ?? [],
      sourceSectionFilterAuditRows
    )
  );
  const allInitialParsedCourseCodes = filterParsedCourseCodesByHints(
    effectiveEntry,
    parsed.snapshotLines ?? [],
    filterExplicitlyExcludedCourseCodes(
      uniqueSorted([
        ...(parsed.courseCodes ?? []),
        ...recoverStructuredCourseCodesFromSourceEvidence(
          effectiveEntry,
          structuredCourseCodes,
          parsed
        ),
      ]),
      explicitlyExcludedCourseCodeSet
    )
  );
  ...
  const rawParsedRequirementGroups = canCreateStructuredRequirementGroups
    ? filterParsedRequirementGroupsByCourseCodePredicate(
        ...
        (courseCode) =>
          !explicitlyExcludedCourseCodeSet.has(normalizeCourseCode(courseCode)) &&
...
function mergeParsedOwnerCanonicalSourceCoverage(left, right) {
  ...
  const mergedSourceSectionFilterAuditRows = [
    ...(left.sourceSectionFilterAuditRows ?? []),
    ...(right.sourceSectionFilterAuditRows ?? []),
  ];
  const mergedParserSequenceChoiceAuditRows = [
    ...(left.parserSequenceChoiceAuditRows ?? []),
    ...(right.parserSequenceChoiceAuditRows ?? []),
  ];
  return {
    ...selected,
    ...
    parsedUwCourseCodes: uniqueSorted([
      ...(left.parsedUwCourseCodes ?? []),
      ...(right.parsedUwCourseCodes ?? []),
    ]),
    approvedFilterUwCourseCodes: uniqueSorted([
      ...(left.approvedFilterUwCourseCodes ?? []),
      ...(right.approvedFilterUwCourseCodes ?? []),
    ]),
    electiveListUwCourseCodes: uniqueSorted([
      ...(left.electiveListUwCourseCodes ?? []),
      ...(right.electiveListUwCourseCodes ?? []),
    ]),
    supportOnlyUwCourseCodes: uniqueSorted([
      ...(left.supportOnlyUwCourseCodes ?? []),
      ...(right.supportOnlyUwCourseCodes ?? []),
    ]),
    sourceOnlyUwCourseCodes: uniqueSorted([
      ...(left.sourceOnlyUwCourseCodes ?? []),
      ...(right.sourceOnlyUwCourseCodes ?? []),
    ]),
    structuredOnlyUwCourseCodes: uniqueSorted([
      ...(left.structuredOnlyUwCourseCodes ?? []),
      ...(right.structuredOnlyUwCourseCodes ?? []),
    ]),
    requirementCueLines: uniqueInOrder([
      ...(left.requirementCueLines ?? []),
      ...(right.requirementCueLines ?? []),
    ]),
    chooseStatements: uniqueInOrder([
      ...(left.chooseStatements ?? []),
      ...(right.chooseStatements ?? []),
    ]),
    pathwayLabels: uniqueInOrder([
      ...(left.pathwayLabels ?? []),
      ...(right.pathwayLabels ?? []),
    ]),
    supportLists: uniqueBy(
      [...(left.supportLists ?? []), ...(right.supportLists ?? [])],
      getRequirementSupportListSemanticKey
    ),
    parsedRequirementAtomCandidates: uniqueBy(
      [
        ...(left.parsedRequirementAtomCandidates ?? []),
        ...(right.parsedRequirementAtomCandidates ?? []),
      ],
      (candidate) => candidate.id ?? `${candidate.courseCode ?? ""}:${candidate.sourceRowText ?? ""}`
    ),
    parsedDegreeMapBlockCandidates: uniqueBy(
      [
        ...(left.parsedDegreeMapBlockCandidates ?? []),
        ...(right.parsedDegreeMapBlockCandidates ?? []),
      ],
      (candidate) => candidate.id ?? `${candidate.title ?? ""}:${(candidate.uwCourseCodes ?? []).join(",")}`
    ),
    parsedRequirementGroups: uniqueBy(
      [
        ...(left.parsedRequirementGroups ?? []),
        ...(right.parsedRequirementGroups ?? []),
      ],
      (group) => group.id ?? `${group.label ?? ""}:${group.requirementType ?? ""}`
    ),
    parsedRequirementCourses: uniqueBy(
      [
        ...(left.parsedRequirementCourses ?? []),
        ...(right.parsedRequirementCourses ?? []),
      ],
      (course) => course.id ?? `${course.normalizedCourseCode ?? ""}:${course.label ?? ""}`
    ),
    sourceSectionFilterAuditRows: mergedSourceSectionFilterAuditRows,
    sourceSectionFilterAuditLines: mergedSourceSectionFilterAuditRows.map(
      (row) => row.copyOnlyDebugText
    ),
    parserSequenceChoiceAuditRows: mergedParserSequenceChoiceAuditRows,
    parserSequenceChoiceAuditLines: mergedParserSequenceChoiceAuditRows.map(
      (row) => row.copyOnlyDebugText
    ),
    sourceScopeAuditLines: uniqueInOrder([
      ...(left.sourceScopeAuditLines ?? []),
      ...(right.sourceScopeAuditLines ?? []),
    ]),
  };
}
...
function parsedOwnerIsCoveredByChildPathwayRequirements(owner, owners) {
  ...
  const ownerDedupeKey = getParsedOwnerDedupeKey(owner);
  const coveringOwners = (owners ?? []).filter(
    (candidate) =>
      candidate?.planId === owner.planId &&
      candidate !== owner &&
      (!ownerDedupeKey || getParsedOwnerDedupeKey(candidate) !== ownerDedupeKey) &&
      (candidate?.ownerId === owner.ownerId || (!owner.pathwayId && candidate?.pathwayId)) &&
      candidate?.ok &&
      parsedOwnerCanCreateSchedulableRows(candidate) &&
      (candidate.parsedUwCourseCodes ?? []).length > 0
  );
  if (!coveringOwners.length) {
    return false;
  }
...
module.exports = {
  ...
  buildTitleOnlyCourseBackfillSupplementalSourcesForTest:
    buildTitleOnlyCourseBackfillSupplementalSources,
  buildRequirementSourceScope,
  canRequirementSourceRoleCreateSchedulableRows,
  ...
  getInlineSectionCourseTitleForTest: getInlineSectionCourseTitle,
  ...
  getRequirementSourceRoleStatus,
  filterParsedCourseCodesByHintsWithSourceRecoveryForTest:
    filterParsedCourseCodesByHintsWithSourceRecovery,
  ...
  parseLinkedDocumentSourceForTest: parseLinkedDocumentSource,
  ...
  parserRecoverySnapshotIsActiveForOwnerForTest: parserRecoverySnapshotIsActiveForOwner,
  ...
  shouldAllowAlternateManifestEntryForParseForTest: shouldAllowAlternateManifestEntryForParse,
