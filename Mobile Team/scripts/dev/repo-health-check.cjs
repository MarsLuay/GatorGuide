...
function checkRequiredFiles() {
  ...
  return makeFailure(
    ...
    ["Restore the missing files or re-checkout the Mobile Team app folder."]
  );
}
...
function checkBabelCoreInstall() {
  ...
  if (!fileExists(packageJsonPath)) {
    return makeFailure(
      ...
      [],
      ["Run `npm install` from `Mobile Team`."]
    );
  }
...
function checkMetroConfigLoad() {
  ...
  try {
    ...
    if (/tailwind\.config/i.test(message)) {
      fix.push("Make sure `tailwind.config.js` exists at the Mobile Team project root.");
    }
