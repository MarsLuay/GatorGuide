...
} = require("./fixtures/uw-political-law-policy-complete-diagnostics.fixture.cjs");

const RUN_DIAGNOSTICS =
  process.env.TRANSFER_PLANNER_RUN_UW_POLITICAL_LAW_POLICY_DIAGNOSTICS === "1";
...
function normalizeCourseCode(value) {
  return String(value ?? "")
    .replace(/\u00a0/g, " ")
    .toUpperCase()
    .replace(/\bB\s+(BUS|MATH|WRIT)\b/g, "B$1")
    .replace(/\bDIS\s+ST\b/g, "DISST")
    .replace(/\bJSIS\s+([AB])\b/g, "JSIS$1")
    .replace(/\bPOL\s+S\b/g, "POLS")
    .replace(/\bQ\s+(METH|SCI)\b/g, "Q$1")
    .replace(/\bT\s+(AIS|AMST|BECON|COM|CRIM|ECON|EGL|ESC|FILM|GEOG|GH|HIST|LAW|LIT|PHIL|POLS|RELIG|SOC|SPAN|SUD|WOMN|WRT)\b/g, "T$1")
    .replace(/\s+/g, " ")
    .trim()
...
function getDirectPoliticalLawPolicyPlanIds() {
  const planner = getPlanner();
  ...
  return uniqueSorted(
    ["uw-seattle", "uw-bothell", "uw-tacoma"].flatMap((campusId) =>
      planner
        ...
        .filter((plan) => {
          const titles = [plan.title, plan.shortTitle].map(normalizePlanTitle);
...
test("UW political/law/policy complete diagnostic fixture is source scoped and UW-course only", () => {
  assert.deepEqual(
    politicalLawPolicyPrograms.map((program) => program.planId),
    [
  ...
  for (const program of politicalLawPolicyPrograms) {
...
diagnosticTest("UW political/law/policy target roster matches direct official political, law, and policy majors", () => {
  const expectedPlanIds = uniqueSorted(politicalLawPolicyPrograms.map((program) => program.planId));
  const actualPlanIds = getDirectPoliticalLawPolicyPlanIds();

  assert.deepEqual(
    actualPlanIds,
    expectedPlanIds,
    [
      "Direct Political/Law/Policy roster should include Seattle Political Science, Seattle Public Service & Policy, Seattle Law, Societies & Justice, Bothell Law, Economics & Public Policy, Tacoma Law and Policy, and Tacoma Politics, Philosophy and Economics.",
      "Public health, global/international studies, criminal justice, and economics-policy sibling programs should stay outside this target fixture.",
