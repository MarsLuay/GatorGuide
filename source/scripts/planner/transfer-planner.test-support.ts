...
const AUTO_CUSTOM_PREP_FALLBACK_TITLE = "Custom Green River prep";
...
function isHiddenSourceOnlyRuntimeChecklistTitle(title: string) {
  return (
    ...
    title.startsWith(AUTO_UW_PREP_TARGET_PREFIX) ||
...
function getGuideBackedCoverageGaps() {
  ...
  for (const rule of TRANSFER_PLANNER_EQUIVALENCY_RULE_REGISTRY) {
    if (!GUIDE_BACKED_EQUIVALENCY_RULE_KINDS.has(rule.sourceKind ?? "")) {
      continue;
    }
...
const generatedPlan = TRANSFER_PLANNER_GENERATED_MAJOR_PLANS.find(
