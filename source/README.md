...
This folder holds the app source: pages, settings, translations, services, planner tooling, and platform setup.
...
You can run the Playwright-based Windows/web QA harness locally from `source`:
...
This app now deploys to Vercel from `source`:
...
```text
source\.tmp\transfer-planner-maintenance-summary.md
```
