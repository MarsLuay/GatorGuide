...
} = require("./fixtures/uw-public-health-complete-diagnostics.fixture.cjs");

const RUN_DIAGNOSTICS =
  process.env.TRANSFER_PLANNER_RUN_UW_PUBLIC_HEALTH_DIAGNOSTICS === "1";
...
function normalizeCourseCode(value) {
  return String(value ?? "")
    .replace(/\u00a0/g, " ")
    .toUpperCase()
    .replace(/\bB\s+(BIO|BUS|CHEM|EARTH|EDUC|HLTH|HS)\b/g, "B$1")
    .replace(/\bENV\s+H\b/g, "ENVH")
    .replace(/\bG\s+H\b/g, "GH")
    .replace(/\bQ\s+SCI\b/g, "QSCI")
    .replace(/\bT\s+(ELEC|HLTH)\b/g, "T$1")
    .replace(/\s+/g, " ")
    .trim()
...
function getDirectPublicHealthPlanIds() {
  const planner = getPlanner();
...
test("UW public health/health complete diagnostic fixture is source scoped and UW-course only", () => {
  assert.deepEqual(
    publicHealthPrograms.map((program) => program.planId),
    [
...
diagnosticTest("UW public health/health target roster matches direct official health-family majors", () => {
  const expectedPlanIds = uniqueSorted(publicHealthPrograms.map((program) => program.planId));
  const actualPlanIds = getDirectPublicHealthPlanIds();

  assert.deepEqual(
    actualPlanIds,
    expectedPlanIds,
    [
      "Direct Public Health/Health-family roster should include Seattle Environmental Public Health, Seattle Public Health-Global Health, Bothell Health Studies, and Tacoma Healthcare Leadership.",
      "Food Systems/Nutrition/Health, Medical Laboratory Science, Biomedical Sciences, and Nursing are adjacent health programs and should stay outside this target fixture.",
...
for (const program of publicHealthPrograms) {
  diagnosticTest(`${program.title} (${program.campusId}) keeps every official source link`, () => {
