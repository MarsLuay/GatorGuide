...
exports.TRANSFER_PLANNER_TRACKS = exports.TRANSFER_PLANNER_CAMPUSES = exports.TRANSFER_PLANNER_STUDENT_RUNTIME_MAJOR_PLANS = exports.TRANSFER_PLANNER_SOURCE_GENERATED_MAJOR_PLANS = void 0;
exports.isTransferPlannerStudentHiddenSourceGap = isTransferPlannerStudentHiddenSourceGap;
exports.getTransferPlannerAutoMatchedTrackRecommendation = getTransferPlannerAutoMatchedTrackRecommendation;
exports.getTransferPlannerSourceGeneratedMajorsForCampus = getTransferPlannerSourceGeneratedMajorsForCampus;
exports.getTransferPlannerStudentVisibleSourceGeneratedMajorsForCampus = getTransferPlannerStudentVisibleSourceGeneratedMajorsForCampus;
exports.getTransferPlannerStudentVisibleMajorsForCampus = getTransferPlannerStudentVisibleMajorsForCampus;
exports.getTransferPlannerStudentRuntimeMajorsForCampus = getTransferPlannerStudentRuntimeMajorsForCampus;
exports.getTransferPlannerSourceGeneratedMajorPlan = getTransferPlannerSourceGeneratedMajorPlan;
exports.getTransferPlannerMajorsForCampus = getTransferPlannerMajorsForCampus;
exports.getTransferPlannerMajorPlan = getTransferPlannerMajorPlan;
exports.getTransferPlannerStudentRuntimeMajorPlan = getTransferPlannerStudentRuntimeMajorPlan;
exports.getTransferPlannerPathwaysForPlan = getTransferPlannerPathwaysForPlan;
exports.getTransferPlannerStudentVisiblePathwaysForPlan = getTransferPlannerStudentVisiblePathwaysForPlan;
exports.getTransferPlannerStudentRuntimePathwaysForPlan = getTransferPlannerStudentRuntimePathwaysForPlan;
exports.resolveTransferPlannerMajorPlan = resolveTransferPlannerMajorPlan;
exports.resolveTransferPlannerStudentRuntimeMajorPlan = resolveTransferPlannerStudentRuntimeMajorPlan;
exports.getTransferPlannerGrcCourseList = getTransferPlannerGrcCourseList;
exports.getTransferPlannerGrcCourseListGuidance = getTransferPlannerGrcCourseListGuidance;
exports.getTransferPlannerTrack = getTransferPlannerTrack;
exports.getTransferPlannerChainsForPlan = getTransferPlannerChainsForPlan;
exports.getTransferPlannerChainLabel = getTransferPlannerChainLabel;
exports.getTransferPlannerGrcCourseLatestPublishedQuarters = getTransferPlannerGrcCourseLatestPublishedQuarters;
exports.getTransferPlannerGrcCourseAvailability = getTransferPlannerGrcCourseAvailability;
exports.getTransferPlannerGrcCourseAvailabilitySummary = getTransferPlannerGrcCourseAvailabilitySummary;
const bootstrap_generated_1 = require("./bootstrap.generated");
const transfer_planner_grc_availability_generated_1 = require("../transfer-planner-grc-availability.generated");
const transfer_planner_master_generated_1 = require("../transfer-planner-master-generated");
const registry_1 = require("./registry");
const source_gaps_generated_1 = require("./source-gaps.generated");
...
const TRANSFER_PLANNER_CHAIN_LABELS = {
...
const AUTO_MATCH_EXCLUDED_TRACK_TERM_LABEL_PATTERN = /\b(transferability of credits|generally transferable courses|section [a-z])\b/i;
const REQUIRED_FOR_DEGREE_EITHER_WAY_NOTE = "Not part of the minimum transfer-admission classes, but good to complete before or during UW enrollment because it's needed to complete the degree either way.";
const AUTO_FALLBACK_CORE_LABEL_PATTERN = /\b(MATH|PHYS|CHEM|BIOL|ENGR|CS|STAT|ECON|ENGL|CMST|GIS|GEOG|LANG|JAPN|CHIN|SPAN|FRCH|GERM|LATIN|GREEK|MUSC|ART|DESN|DRMA)\b/i;
const AUTO_FALLBACK_CHECKLIST_NOTE = "Auto-generated from the current source-backed Green River class list for this major.";
const AUTO_CUSTOM_PREP_FALLBACK_NOTE = "Use the current source-backed Green River class list as the planning starting point. Unsupported class mixes stay hidden until public sources can verify them.";
const AUTO_SOURCE_BACKED_UW_PREP_TARGET_PREFIX = "UW prep target:";
const AUTO_SOURCE_BACKED_UW_PREP_TARGET_NOTE = "Official UW prep target found in the current source-backed requirements, but no public Green River equivalent is currently provable.";
const AUTO_SOURCE_BACKED_UW_PREP_GUIDANCE_TITLE = "Source-backed UW prep guidance";
const AUTO_SOURCE_BACKED_UW_PREP_GUIDANCE_NOTE = "Current official source-backed requirement materials identify more UW prep for this major, but the published Green River equivalent path is not yet provable enough for planner-ready course mapping.";
...
function makePathwayPlanKey(planId, pathwayId) {
    return `${planId}::${String(pathwayId ?? "")}`;
}
const STUDENT_HIDDEN_SOURCE_GAP_PLAN_IDS = new Set(source_gaps_generated_1.TRANSFER_PLANNER_SOURCE_GAP_ENTRIES.filter((entry) => entry.studentVisibility === "hidden" && !entry.pathwayId).map((entry) => entry.planId));
const STUDENT_HIDDEN_SOURCE_GAP_PATHWAY_KEYS = new Set(source_gaps_generated_1.TRANSFER_PLANNER_SOURCE_GAP_ENTRIES.filter((entry) => entry.studentVisibility === "hidden" && entry.pathwayId).map((entry) => makePathwayPlanKey(entry.planId, entry.pathwayId)));
function isTransferPlannerStudentHiddenSourceGap(planId, pathwayId) {
    if (!planId)
        return false;
    if (pathwayId) {
...
function uniquePlannerStrings(values) {
    const seen = new Set();
    const uniqueValues = [];
    for (const value of values) {
...
function compareGuideRules(left, right) {
    const referenceOnlyDelta = Number(isReferenceOnlyGuideRule(left)) - Number(isReferenceOnlyGuideRule(right));
    ...
    if (statusDelta !== 0)
        return statusDelta;
    const acceptanceDelta = getGuideRuleAcceptanceScore(right) - getGuideRuleAcceptanceScore(left);
    if (acceptanceDelta !== 0)
        return acceptanceDelta;
    const typeDelta = getGuideRuleTypeScore(right) - getGuideRuleTypeScore(left);
    if (typeDelta !== 0)
        return typeDelta;
    const sourceSetLengthDelta = (left.sourceCourseSets?.[0]?.length ?? Number.MAX_SAFE_INTEGER) -
        (right.sourceCourseSets?.[0]?.length ?? Number.MAX_SAFE_INTEGER);
    if (sourceSetLengthDelta !== 0)
        return sourceSetLengthDelta;
    return left.id.localeCompare(right.id);
}
const GUIDE_RULES_BY_TARGET_COURSE_CODE = new Map();
for (const rule of registry_1.TRANSFER_PLANNER_EQUIVALENCY_RULE_REGISTRY) {
...
const SOURCE_BACKED_GUIDE_COURSES_BY_KEY = new Map();
for (const parsedSource of registry_1.TRANSFER_PLANNER_PARSED_REQUIREMENT_SOURCE_BLOCK_REGISTRY) {
    const scopeKey = makePathwayPlanKey(parsedSource.planId, parsedSource.pathwayId);
    const sourceBackedGuideCourseCodes = new Set(SOURCE_BACKED_GUIDE_COURSES_BY_KEY.get(scopeKey) ?? []);
    const parsedCourseCodes = uniquePlannerStrings([...(parsedSource.parsedUwCourseCodes ?? []), ...(parsedSource.sourceOnlyUwCourseCodes ?? [])].map((courseCode) => normalizeCourseCode(courseCode)));
    ...
    SOURCE_BACKED_GUIDE_COURSES_BY_KEY.set(scopeKey, uniquePlannerStrings([...sourceBackedGuideCourseCodes]));
}
const SOURCE_BACKED_CLASSIFICATION_COURSES_BY_KEY = new Map();
for (const classification of registry_1.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY) {
    const scopeKey = makePathwayPlanKey(classification.planId, classification.pathwayId);
    const sourceBackedClassificationCourseCodes = new Set(SOURCE_BACKED_CLASSIFICATION_COURSES_BY_KEY.get(scopeKey) ?? []);
    ...
    SOURCE_BACKED_CLASSIFICATION_COURSES_BY_KEY.set(scopeKey, uniquePlannerStrings([...sourceBackedClassificationCourseCodes]));
}
...
function sanitizeDegreeMapSection(section) {
    return {
        ...section,
...
function uniquePlannerLinks(values) {
    const seen = new Set();
    const uniqueValues = [];
    for (const value of values) {
        const key = `${String(value.url ?? "").trim()}|${String(value.label ?? "").trim()}|${String(value.note ?? "").trim()}`;
        if (!key || seen.has(key))
            continue;
        seen.add(key);
        uniqueValues.push(value);
...
function uniqueReferenceCourseLabels(items) {
    const seen = new Set();
    const labels = [];
...
function extractReferenceCourseCodes(value) {
    return uniqueReferenceCourseLabels((String(value ?? "").toUpperCase().match(REFERENCE_COURSE_CODE_PATTERN) ?? []).map((match) => match.replace(/\s+/g, " ").trim()));
}
function getChecklistReferenceCoursesFromItems(items) {
    return uniqueReferenceCourseLabels(items.flatMap((item) => [item.grcCourses, ...(item.alternatives ?? [])].flat()));
}
function normalizeChecklistItems(items) {
    return items ?? [];
}
function getChecklistReferenceCourses(plan) {
...
function orderByBaseIds(entries, baseIds) {
    const order = new Map(baseIds.map((id, index) => [id, index]));
...
function orderStringsByBase(values, baseValues) {
    const order = new Map(baseValues.map((value, index) => [normalizeCourseCode(value), index]));
...
function orderLinksByBase(links, baseLinks) {
    const order = new Map(baseLinks.map((link, index) => [link.url, index]));
...
function extractLeafId(id) {
    return String(id ?? "").split(":").pop() ?? id;
}
function buildChecklistItemSignature(item) {
    return JSON.stringify({
        title: sanitizePlannerOwnedText(item.title),
        grcCourses: item.grcCourses.map((code) => normalizeCourseCode(code)),
        alternatives: (item.alternatives ?? []).map((group) => group.map((code) => normalizeCourseCode(code))),
        minCompletedCount: item.minCompletedCount ?? null,
    });
}
const CANONICAL_GRC_COURSE_BY_CODE = new Map(registry_1.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.schoolId === "grc").map((entry) => [normalizeCourseCode(entry.code), entry]));
const GRC_PREREQUISITE_CLOSURE_CACHE = new Map();
function getTransitiveGrcDependencyCourseCodes(courseCode) {
...
function pruneRuntimeChecklistCourseGroup(group, supportedCourseCodes) {
    const normalizedGroup = uniqueReferenceCourseLabels(group);
    const directlySupportedCodes = normalizedGroup.filter((code) => supportedCourseCodes.has(normalizeCourseCode(code)));
    ...
    while (changed) {
        ...
        for (const code of normalizedGroup) {
            ...
            const isDependencyOfKeptCode = [...keptCodes].some((keptCode) => getTransitiveGrcDependencyCourseCodes(keptCode).has(normalizedCode));
...
function buildSourceValidatedRuntimeChecklistItem(atom, supportedCourseCodes) {
...
function getChecklistCoverageCourseCodes(items) {
    return uniqueReferenceCourseLabels(items.flatMap((item) => [item.grcCourses, ...(item.alternatives ?? [])].flat())).map((code) => normalizeCourseCode(code));
}
function pruneRedundantRuntimeChecklistItems(items, coveredCourseCodes) {
    const coverage = new Set([...coveredCourseCodes].map((code) => normalizeCourseCode(code)).filter(Boolean));
    const prunedItems = [];
    for (const item of items) {
...
function getRuntimeRequirementAtoms(planId, pathwayId) {
    return uniqueById([
...
function buildChecklistItem(atom) {
    return sanitizeChecklistItem({
        id: extractLeafId(atom.id),
        title: atom.title,
...
function hasAnyChecklistItems(scope) {
    return Boolean((scope.applicationChecklist?.length ?? 0) > 0 ||
        (scope.beforeEnrollmentChecklist?.length ?? 0) > 0 ||
        (scope.stayAtGrcChecklist?.length ?? 0) > 0);
}
function buildAutoChecklistItemId(label, index) {
...
function buildAutoFallbackCourseLabels(grcCourseList, bestTrackId) {
    ...
    const firstLabelByCode = new Map();
    ...
    const trackSeedLabels = preferredTrack
        ? uniquePlannerStrings(preferredTrack.terms.flatMap((term) => term.courses.flatMap((courseLabel) => extractReferenceCourseCodes(courseLabel)
    ...
    return uniquePlannerStrings([...trackSeedLabels, ...coreLabels, ...courseList]).slice(0, AUTO_FALLBACK_CHECKLIST_LIMIT);
}
function buildAutoFallbackChecklist(scope) {
    ...
    if (autoCourseLabels.length) {
        return autoCourseLabels.map((label, index) => ({
...
function buildEmptySourceBackedFallbackChecklists() {
    return {
        beforeApplication: [],
        beforeEnrollment: [],
        stayAtGrc: [],
...
function getRequirementDiffClassificationsForScope(planId, pathwayId) {
    return registry_1.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY.filter((entry) => entry.planId === planId &&
        (pathwayId === undefined
            ? true
            : pathwayId === null
                ? !entry.pathwayId
...
function getParsedRequirementAtomCandidatesForScope(planId, pathwayId) {
    return registry_1.TRANSFER_PLANNER_PARSED_REQUIREMENT_SOURCE_BLOCK_REGISTRY.filter((entry) => entry.planId === planId &&
        (pathwayId === undefined
            ? true
            : pathwayId === null
                ? !entry.pathwayId
                : entry.pathwayId === pathwayId)).flatMap((entry) => entry.parsedRequirementAtomCandidates);
}
function getRequirementCueLinesFromClassification(entry) {
...
function getSourceBackedFallbackCourseLevel(code) {
    const match = normalizeCourseCode(code).match(/(\d{3})[A-Z]?$/);
    return match ? Number(match[1]) : null;
}
function getSourceBackedFallbackCoursePrefix(code) {
...
function getSourceBackedFallbackCodeIdentity(code) {
    return normalizeCourseCode(code).replace(/\s+/g, "");
}
function countSourceBackedFallbackPrefixSpaces(code) {
    return (getSourceBackedFallbackCoursePrefix(code)
        .match(/\s+/g)
        ?.length ?? 0);
}
function shouldPreferSourceBackedFallbackDisplayCode(candidateCode, existingCode) {
...
function pickPreferredSourceBackedFallbackDisplayCode(...codes) {
    let bestCode = "";
...
function isLowerDivisionSourceBackedFallbackCode(code) {
...
function shouldExcludeSourceBackedFallbackCode(scope) {
    ...
    if (/\bmay also be helpful\b/i.test(combinedCueText) ||
    ...
    if (prefix === "LANGUAGE") {
        const sameLevelSpecificCodes = [...scope.allCandidateCodes].filter((candidateCode) => {
            ...
            return !SOURCE_ONLY_FALLBACK_NOISE_PREFIXES.has(getSourceBackedFallbackCoursePrefix(candidateCode));
        });
...
function getDominantSourceBackedFallbackPhase(entries, fallbackPhase = "before-enrollment") {
...
function addFallbackChecklistItemForPhase(checklists, phase, item) {
...
function appendUniqueChecklistItems(existingItems, addedItems) {
...
function buildSourceBackedFallbackGuidanceChecklists(phase, scope) {
...
function buildStrictSourceBackedFallbackChecklists(scope) {
    ...
    const supportsStrictFallback = classifications.length > 0 &&
        classifications.every((entry) => entry.classificationKind === "source-backed-choice-set-no-public-grc-path" &&
    ...
    const supportsParsedFallback = scope.includeParsedCandidates && (candidateByCode.size > 0 || classificationByCode.size > 0);
    if (!supportsStrictFallback && !supportsParsedFallback) {
        return buildEmptySourceBackedFallbackChecklists();
    }
    const codeIdentitiesToInspect = new Set();
    if (supportsStrictFallback || supportsParsedFallback) {
    ...
    const dominantPhase = getDominantSourceBackedFallbackPhase(classifications, supportsStrictFallback ? "before-enrollment" : "stay-at-grc");
    const checklists = buildEmptySourceBackedFallbackChecklists();
    const orderedCodeIdentities = [...codeIdentitiesToInspect].sort((left, right) => {
        const leftCode = pickPreferredSourceBackedFallbackDisplayCode(candidateByCode.get(left)?.displayCode, classificationByCode.get(left)?.displayCode, left);
        const rightCode = pickPreferredSourceBackedFallbackDisplayCode(candidateByCode.get(right)?.displayCode, classificationByCode.get(right)?.displayCode, right);
        const levelDelta = (getSourceBackedFallbackCourseLevel(leftCode) ?? Number.MAX_SAFE_INTEGER) -
    ...
    for (const codeIdentity of orderedCodeIdentities) {
        ...
        const normalizedCode = pickPreferredSourceBackedFallbackDisplayCode(candidateRecord?.displayCode, classificationRecord?.displayCode, codeIdentity);
        ...
        if (shouldExcludeSourceBackedFallbackCode({
            code: normalizedCode,
            requirementCueLines,
            allCandidateCodes: new Set([...codeIdentitiesToInspect]
                .map((identity) => pickPreferredSourceBackedFallbackDisplayCode(candidateByCode.get(identity)?.displayCode, classificationByCode.get(identity)?.displayCode, identity))
                .filter(Boolean)),
        })) {
            continue;
        }
    ...
    const totalFallbackTargets = checklists.beforeApplication.length +
    ...
    if (checklists.beforeApplication.length ||
        checklists.beforeEnrollment.length ||
        checklists.stayAtGrc.length) {
        return checklists;
    }
...
function applyStrictSourceBackedFallback(scope, pathwayId) {
    ...
    if (!fallbackChecklists.beforeApplication.length &&
    ...
    return {
        ...scope,
        applicationChecklist: appendUniqueChecklistItems(scope.applicationChecklist, fallbackChecklists.beforeApplication),
        beforeEnrollmentChecklist: appendUniqueChecklistItems(scope.beforeEnrollmentChecklist, fallbackChecklists.beforeEnrollment),
        stayAtGrcChecklist: appendUniqueChecklistItems(scope.stayAtGrcChecklist, fallbackChecklists.stayAtGrc),
    };
}
function collectPlannerCourseLabels(scope) {
    const checklistLabels = uniqueReferenceCourseLabels(getChecklistReferenceCoursesFromItems([
...
function buildReferenceLabelByCode(labels) {
...
function shouldUseTrackTermForAutoMatch(termLabel) {
    return !AUTO_MATCH_EXCLUDED_TRACK_TERM_LABEL_PATTERN.test(String(termLabel ?? "").trim());
}
function buildTrackReferenceCourseCodes(track, options = {}) {
    const includeCatalogYears = options.includeCatalogYears ?? true;
    const filterTerms = (terms) => (options.autoMatchOnly
...
function buildAutoTrackFinancialAidNote(track) {
...
function getTransferPlannerAutoMatchedTrackRecommendation(grcCourseList, preferredTrackId = null) {
    const normalizedCourseLabels = uniqueReferenceCourseLabels(grcCourseList);
    const planCourseCodes = uniqueReferenceCourseLabels(normalizedCourseLabels.flatMap((label) => extractReferenceCourseCodes(label)));
    ...
    const scoredTracks = bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_TRACKS.map((track) => {
        ...
        return {
            ...
            trackCourseCodes,
            matchedCourseCodes,
            matchedCourseLabels: uniquePlannerStrings(matchedCourseCodes.map((code) => labelByCode.get(code) ?? code)),
...
function applyAutoTrackRecommendation(scope) {
    const autoTrack = getTransferPlannerAutoMatchedTrackRecommendation(collectPlannerCourseLabels(scope), scope.bestTrackId ?? null);
...
function applyAutoChecklistFallback(scope) {
...
const REQUIREMENTS_BY_KEY = new Map();
for (const requirement of registry_1.TRANSFER_PLANNER_MAJOR_REQUIREMENT_REGISTRY) {
...
const DEGREE_MAPS_BY_KEY = new Map();
for (const block of registry_1.TRANSFER_PLANNER_DEGREE_MAP_BLOCK_REGISTRY) {
...
const POLICIES_BY_KEY = new Map();
for (const policy of registry_1.TRANSFER_PLANNER_POLICY_REGISTRY) {
    POLICIES_BY_KEY.set(makePathwayPlanKey(policy.planId, policy.pathwayId), policy);
}
const PATHWAYS_BY_PLAN = new Map();
for (const pathway of registry_1.TRANSFER_PLANNER_MAJOR_PATHWAY_REGISTRY) {
    const current = PATHWAYS_BY_PLAN.get(pathway.planId) ?? [];
    current.push(pathway);
    PATHWAYS_BY_PLAN.set(pathway.planId, current);
}
function getStructuredCourseCodesForPlan(planId, baseCourseOrder, pathwayId) {
    ...
    const filteredCodes = registry_1.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.schoolId === "grc" &&
        entry.referencedByPlanIds.includes(planId) &&
        entry.sourceKinds.some((kind) => STRUCTURED_GRC_SOURCE_KINDS.has(kind)) &&
        (pathwayId
            ? entry.sourceContexts.some((context) => context.includes(`:pathway:${pathwayId}:`) || !context.includes(":pathway:"))
            : !entry.sourceContexts.some((context) => context.includes(":pathway:")))).map((entry) => entry.code);
    const sourceBackedGuideCodes = scopeKeys.flatMap((scopeKey) => SOURCE_BACKED_GUIDE_COURSES_BY_KEY.get(scopeKey) ?? []);
    const sourceBackedClassificationCodes = scopeKeys.flatMap((scopeKey) => SOURCE_BACKED_CLASSIFICATION_COURSES_BY_KEY.get(scopeKey) ?? []);
    return orderStringsByBase(uniquePlannerStrings([
        ...baseCourseOrder,
        ...filteredCodes,
...
function collectStructuredLinks(planId, baseLinks, pathwayId) {
    const key = makePathwayPlanKey(planId, pathwayId);
    const links = uniquePlannerLinks(compact([
        ...baseLinks,
        ...
        ...(pathwayId
            ...
            : []).flatMap((entry) => entry.sourceLinks.map(toPlannerLink)),
    ]));
    return orderLinksByBase(links, baseLinks);
}
function collectStructuredValidationNotes(planId, baseNotes, pathwayId) {
    const key = makePathwayPlanKey(planId, pathwayId);
    const structuredNotes = uniquePlannerStrings([
        ...
        ...(pathwayId
            ? PATHWAYS_BY_PLAN.get(planId)?.filter((entry) => entry.pathwayId === pathwayId) ?? []
            : []).flatMap((entry) => entry.validationNotes),
    ]);
...
function buildChecklistForPhase(planId, phase, baseItems, pathwayId) {
    ...
    return orderByBaseIds(items.map(buildChecklistItem), baseItems.map((item) => item.id));
}
function buildDegreeMapSections(planId, baseSections, pathwayId) {
    ...
    return orderByBaseIds(blocks.map(buildDegreeMapSection), (baseSections ?? []).map((section) => section.id));
}
function buildPathway(basePlan, basePathway) {
    const key = makePathwayPlanKey(basePlan.id, basePathway.id);
    const policy = POLICIES_BY_KEY.get(key);
    const registryPathway = PATHWAYS_BY_PLAN.get(basePlan.id)?.find((entry) => entry.pathwayId === basePathway.id) ?? null;
    const applicationChecklist = buildChecklistForPhase(basePlan.id, "before-application", basePathway.applicationChecklist ?? [], basePathway.id);
    return applyAutoChecklistFallback(applyAutoTrackRecommendation({
        ...
        beforeEnrollmentChecklist: buildChecklistForPhase(basePlan.id, "before-enrollment", basePathway.beforeEnrollmentChecklist ?? [], basePathway.id),
        stayAtGrcChecklist: buildChecklistForPhase(basePlan.id, "stay-at-grc", basePathway.stayAtGrcChecklist ?? [], basePathway.id),
        advisorFlags: sanitizePlannerOwnedStrings(policy?.advisorFlags ?? basePathway.advisorFlags ?? []),
        officialLinks: collectStructuredLinks(basePlan.id, basePathway.officialLinks ?? basePlan.officialLinks, basePathway.id),
        degreeMapSections: buildDegreeMapSections(basePlan.id, basePathway.degreeMapSections, basePathway.id),
        manualReviewNotes: sanitizePlannerOwnedStrings(collectStructuredValidationNotes(basePlan.id, basePathway.manualReviewNotes ?? basePlan.manualReviewNotes ?? [], basePathway.id)),
        grcCourseList: getStructuredCourseCodesForPlan(basePlan.id, uniqueReferenceCourseLabels([
            ...(basePlan.grcCourseList ?? []),
            ...(basePathway.grcCourseList ?? []),
        ]), basePathway.id),
        grcCourseListGuidance: sanitizePlannerOwnedText(policy?.grcCourseListGuidance ?? basePathway.grcCourseListGuidance),
        plannerNote: sanitizePlannerOwnedText(policy?.plannerNote ?? basePathway.plannerNote),
        bestTrackId: policy?.bestTrackId === undefined ? basePathway.bestTrackId : policy.bestTrackId,
        bestTrackSummary: sanitizePlannerOwnedText(policy?.bestTrackSummary ?? basePathway.bestTrackSummary),
        whyThisTrack: sanitizePlannerOwnedStrings(policy?.whyThisTrack.length ? [...policy.whyThisTrack] : [...(basePathway.whyThisTrack ?? [])]),
        financialAidNote: sanitizePlannerOwnedText(policy?.financialAidNote ?? basePathway.financialAidNote),
    }));
}
function buildSourceGeneratedPlan(basePlan) {
    const policy = POLICIES_BY_KEY.get(makePathwayPlanKey(basePlan.id, null));
    const applicationChecklist = buildChecklistForPhase(basePlan.id, "before-application", basePlan.applicationChecklist);
    return applyAutoChecklistFallback(applyAutoTrackRecommendation({
        ...basePlan,
        ...
        whyThisTrack: sanitizePlannerOwnedStrings(policy?.whyThisTrack.length ? [...policy.whyThisTrack] : [...basePlan.whyThisTrack]),
        financialAidNote: sanitizePlannerOwnedText(policy?.financialAidNote ?? basePlan.financialAidNote),
        applicationChecklist,
        beforeEnrollmentChecklist: buildChecklistForPhase(basePlan.id, "before-enrollment", basePlan.beforeEnrollmentChecklist),
        stayAtGrcChecklist: buildChecklistForPhase(basePlan.id, "stay-at-grc", basePlan.stayAtGrcChecklist),
        ...
        manualReviewNotes: sanitizePlannerOwnedStrings(collectStructuredValidationNotes(basePlan.id, basePlan.manualReviewNotes ?? [])),
        grcCourseList: getStructuredCourseCodesForPlan(basePlan.id, basePlan.grcCourseList ?? []),
        grcCourseListGuidance: sanitizePlannerOwnedText(policy?.grcCourseListGuidance ?? basePlan.grcCourseListGuidance),
        plannerNote: sanitizePlannerOwnedText(policy?.plannerNote ?? basePlan.plannerNote),
        pathways: orderByBaseIds((basePlan.pathways ?? []).map((pathway) => buildPathway(basePlan, pathway)), (basePlan.pathways ?? []).map((pathway) => pathway.id)),
    }));
}
function getAutomaticScopeKeys(planId, pathwayId) {
    return pathwayId
        ? [makePathwayPlanKey(planId, null), makePathwayPlanKey(planId, pathwayId)]
        : [makePathwayPlanKey(planId, null)];
}
function getAutomaticPromotedRequirementAtoms(planId, pathwayId) {
    return uniqueById([
        ...(0, registry_1.getTransferPlannerPromotedRequirementAtomOverrides)(planId, null),
        ...(pathwayId ? (0, registry_1.getTransferPlannerPromotedRequirementAtomOverrides)(planId, pathwayId) : []),
    ]);
}
function buildAutomaticChecklistForPhase(planId, phase, pathwayId) {
    const supportedCourseCodes = new Set(buildAutomaticCourseList(planId, pathwayId).map((code) => normalizeCourseCode(code)));
...
function buildAutomaticCourseList(planId, pathwayId) {
    const scopeKeys = getAutomaticScopeKeys(planId, pathwayId);
    const overrideCourseCodes = getAutomaticPromotedRequirementAtoms(planId, pathwayId).flatMap((entry) => [
        ...entry.grcCourseCodes,
        ...(entry.alternativeCourseCodeSets ?? []).flat(),
    ]);
    return orderStringsByBase(uniquePlannerStrings([
        ...overrideCourseCodes,
        ...scopeKeys.flatMap((scopeKey) => SOURCE_BACKED_CLASSIFICATION_COURSES_BY_KEY.get(scopeKey) ?? []),
        ...scopeKeys.flatMap((scopeKey) => SOURCE_BACKED_GUIDE_COURSES_BY_KEY.get(scopeKey) ?? []),
    ]), []);
}
function buildStudentVisibleAutomaticCourseList(scope) {
    return collectPlannerCourseLabels(scope);
}
function buildStudentRuntimePathway(basePlan, basePathway) {
    const applicationChecklist = buildAutomaticChecklistForPhase(basePlan.id, "before-application", basePathway.id);
    const beforeEnrollmentChecklist = buildAutomaticChecklistForPhase(basePlan.id, "before-enrollment", basePathway.id);
    const prunedBeforeEnrollmentChecklist = pruneRedundantRuntimeChecklistItems(beforeEnrollmentChecklist, getChecklistCoverageCourseCodes(applicationChecklist));
    const stayAtGrcChecklist = buildAutomaticChecklistForPhase(basePlan.id, "stay-at-grc", basePathway.id);
    const prunedStayAtGrcChecklist = pruneRedundantRuntimeChecklistItems(stayAtGrcChecklist, getChecklistCoverageCourseCodes([
        ...applicationChecklist,
    ...
    return applyAutoChecklistFallback(applyStrictSourceBackedFallback(applyAutoTrackRecommendation({
        id: basePathway.id,
...
function buildStudentRuntimePlan(basePlan) {
    const applicationChecklist = buildAutomaticChecklistForPhase(basePlan.id, "before-application");
    const beforeEnrollmentChecklist = buildAutomaticChecklistForPhase(basePlan.id, "before-enrollment");
    const prunedBeforeEnrollmentChecklist = pruneRedundantRuntimeChecklistItems(beforeEnrollmentChecklist, getChecklistCoverageCourseCodes(applicationChecklist));
    ...
    return applyAutoChecklistFallback(applyStrictSourceBackedFallback(applyAutoTrackRecommendation({
        ...
        grcCourseListGuidance: undefined,
        plannerNote: undefined,
        pathways: orderByBaseIds((basePlan.pathways ?? []).map((pathway) => buildStudentRuntimePathway(basePlan, pathway)), (basePlan.pathways ?? []).map((pathway) => pathway.id)),
    }), null));
}
function materializePlannerPathway(pathway) {
    return {
        ...pathway,
...
function materializePlanPathways(plan, includeHiddenSourceGaps = true) {
    const pathways = (plan.pathways ?? []).map(materializePlannerPathway);
    if (includeHiddenSourceGaps) {
        return pathways;
    }
    return pathways.filter((pathway) => !isTransferPlannerStudentHiddenSourceGap(plan.id, pathway.id));
}
function materializePlanReferenceCourses(plan) {
    return {
        ...plan,
...
function mergePlannerPathwayWithPlan(plan, pathway, visiblePathways = materializePlanPathways(plan)) {
    const mergedPlan = materializePlanReferenceCourses({
        ...
        beforeEnrollmentChecklist: pathway.beforeEnrollmentChecklist ?? plan.beforeEnrollmentChecklist ?? [],
        ...
        degreeMapSections: (pathway.degreeMapSections ?? plan.degreeMapSections)?.map((section) => sanitizeDegreeMapSection(section)),
        manualReviewNotes: sanitizePlannerOwnedStrings([
            ...(plan.manualReviewNotes ?? []),
            ...(pathway.manualReviewNotes ?? []),
        ]),
        grcCourseListGuidance: sanitizePlannerOwnedText(pathway.grcCourseListGuidance ?? plan.grcCourseListGuidance),
        grcCourseList: pathway.grcCourseList && pathway.grcCourseList.length
            ? pathway.grcCourseList
            : plan.grcCourseList,
        ...
        whyThisTrack: sanitizePlannerOwnedStrings(pathway.whyThisTrack?.length ? pathway.whyThisTrack : plan.whyThisTrack),
        financialAidNote: sanitizePlannerOwnedText(pathway.financialAidNote ?? plan.financialAidNote),
    });
    return {
        ...mergedPlan,
...
const TRACKS_BY_ID = new Map(bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_TRACKS.map((track) => [track.id, track]));
const TRACK_REFERENCE_CODES_BY_ID = new Map(bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_TRACKS.map((track) => [
    track.id,
...
const CHAINS_BY_ID = new Map(transfer_planner_master_generated_1.TRANSFER_PLANNER_MASTER_CHAIN_LIBRARY.map((chain) => [chain.id, chain]));
const GRC_AVAILABILITY_BY_CODE = transfer_planner_grc_availability_generated_1.TRANSFER_PLANNER_GRC_COURSE_AVAILABILITY;
function formatAvailabilitySourceWindow(availability) {
    const labels = availability.years.map((year) => year.label).filter(Boolean);
    if (!labels.length)
        return "the latest published Green River annual schedules";
    if (labels.length === 1)
        return `the latest published ${labels[0]} Green River annual schedule`;
    if (labels.length === 2) {
...
function formatAvailabilityStatusSummary(availability) {
    const yearSummaries = availability.years
        .filter((year) => year.quarters.length > 0)
        .map((year) => `${year.label}: ${year.quarters
...
exports.TRANSFER_PLANNER_SOURCE_GENERATED_MAJOR_PLANS = bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_ALL_MAJOR_PLANS.map(buildSourceGeneratedPlan);
exports.TRANSFER_PLANNER_STUDENT_RUNTIME_MAJOR_PLANS = bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_ALL_MAJOR_PLANS.map(buildStudentRuntimePlan);
exports.TRANSFER_PLANNER_CAMPUSES = bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_CAMPUSES;
exports.TRANSFER_PLANNER_TRACKS = bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_TRACKS;
function getTransferPlannerSourceGeneratedMajorsForCampus(campusId) {
    return exports.TRANSFER_PLANNER_SOURCE_GENERATED_MAJOR_PLANS.filter((plan) => plan.campusId === campusId);
}
function getTransferPlannerStudentVisibleSourceGeneratedMajorsForCampus(campusId) {
    return getTransferPlannerSourceGeneratedMajorsForCampus(campusId).filter((plan) => !isTransferPlannerStudentHiddenSourceGap(plan.id));
}
function getTransferPlannerStudentVisibleMajorsForCampus(campusId) {
    return getTransferPlannerStudentVisibleSourceGeneratedMajorsForCampus(campusId);
}
function getTransferPlannerStudentRuntimeMajorsForCampus(campusId) {
    return exports.TRANSFER_PLANNER_STUDENT_RUNTIME_MAJOR_PLANS.filter((plan) => plan.campusId === campusId && !isTransferPlannerStudentHiddenSourceGap(plan.id));
}
function getTransferPlannerSourceGeneratedMajorPlan(planId) {
    return (exports.TRANSFER_PLANNER_SOURCE_GENERATED_MAJOR_PLANS.find((plan) => plan.id === planId) ?? null);
}
function getTransferPlannerMajorsForCampus(campusId) {
    return getTransferPlannerSourceGeneratedMajorsForCampus(campusId);
}
function getTransferPlannerMajorPlan(planId) {
    return getTransferPlannerSourceGeneratedMajorPlan(planId);
}
function getTransferPlannerStudentRuntimeMajorPlan(planId) {
    return (exports.TRANSFER_PLANNER_STUDENT_RUNTIME_MAJOR_PLANS.find((plan) => plan.id === planId) ?? null);
}
function getTransferPlannerPathwaysForPlan(plan) {
    if (!plan?.pathways?.length)
        return [];
    return materializePlanPathways(plan);
}
function getTransferPlannerStudentVisiblePathwaysForPlan(plan) {
    if (!plan?.pathways?.length)
        return [];
    return materializePlanPathways(plan, false);
}
function getTransferPlannerStudentRuntimePathwaysForPlan(plan) {
    if (!plan?.pathways?.length)
        return [];
    return materializePlanPathways(plan, false);
}
function resolveTransferPlannerMajorPlan(plan, pathwayId) {
    if (!plan)
        return null;
...
function resolveTransferPlannerStudentRuntimeMajorPlan(plan, pathwayId) {
    if (!plan)
        return null;
...
function getTransferPlannerGrcCourseList(plan) {
    if (!plan)
        return [];
    return uniqueReferenceCourseLabels([
        ...(plan.grcCourseList ?? []),
...
function getTransferPlannerGrcCourseListGuidance(plan) {
    const guidance = String(plan?.grcCourseListGuidance ?? "").trim();
    return guidance || null;
}
function getTransferPlannerTrack(trackId) {
    if (!trackId)
        return null;
    return TRACKS_BY_ID.get(trackId) ?? null;
}
function getTransferPlannerChainsForPlan(plan) {
    if (!plan?.chainIds?.length)
        return [];
    return plan.chainIds
        .map((chainId) => CHAINS_BY_ID.get(chainId) ?? null)
        .filter((chain) => Boolean(chain));
}
function getTransferPlannerChainLabel(chainId) {
    return TRANSFER_PLANNER_CHAIN_LABELS[chainId] ?? chainId;
}
function getTransferPlannerGrcCourseLatestPublishedQuarters(courseLabel) {
    return getTransferPlannerGrcCourseAvailability(courseLabel)?.latestPublishedQuarters ?? null;
}
function getTransferPlannerGrcCourseAvailability(courseLabel) {
...
function getTransferPlannerGrcCourseAvailabilitySummary(courseLabel) {
