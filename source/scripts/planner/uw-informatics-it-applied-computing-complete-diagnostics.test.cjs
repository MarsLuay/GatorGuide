...
} = require("./fixtures/uw-informatics-it-applied-computing-complete-diagnostics.fixture.cjs");

const RUN_DIAGNOSTICS =
  process.env.TRANSFER_PLANNER_RUN_UW_INFORMATICS_IT_APPLIED_COMPUTING_DIAGNOSTICS === "1";
...
function normalizeCourseCode(value) {
  return String(value ?? "")
    ...
    .replace(/\bT\s+(CSS|INFO|MATH|WRT)\b/g, "T$1")
    .replace(/\s+/g, " ")
    .trim()
...
function getDirectInformationComputingPlanIds() {
  const planner = getPlanner();
  ...
  const excludedTitles = new Set([
    ...
    "computer science and systems: bachelor of arts",
    "computer science and systems: bachelor of science",
    "data visualization",
...
test("UW informatics/IT/applied computing complete diagnostic fixture is source scoped and UW-course only", () => {
  assert.deepEqual(
    informationComputingPrograms.map((program) => program.planId),
    [
  ...
  for (const program of informationComputingPrograms) {
...
diagnosticTest("UW informatics/IT/applied computing target roster matches direct official information-computing majors", () => {
  const expectedPlanIds = uniqueSorted(informationComputingPrograms.map((program) => program.planId));
  const actualPlanIds = getDirectInformationComputingPlanIds();

  assert.deepEqual(
    actualPlanIds,
    expectedPlanIds,
    [
      "Direct Informatics/IT/Applied Computing roster should include Seattle Informatics, Bothell Applied Computing, and Tacoma Information Technology.",
      "CS/CE, Data Visualization, Interactive Media Design, and Science, Technology & Society are neighboring programs and should stay outside this target fixture.",
