...
Status: current implementation as reflected by the Mobile Team client code on 2026-03-28.
...
| Profile fields (`name`, `major`, `gpa`, `state`, `resume`, `transcript`, `avatar`, residency and English fields) | Local mirror in `gatorguide:appdata:v1.user` | Firestore `users/{uid}` | Firestore for signed-in users; local only for guests |
