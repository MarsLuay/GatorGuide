...
function normalizeCourseCode(value) {
  return String(value ?? "")
    .replace(/\u00a0/g, " ")
    .toUpperCase()
    .replace(/\bB\s+(HLTH|NURS)\b/g, "B$1")
    .replace(/\bN\s+(CLIN|METH)\b/g, "N$1")
    .replace(/\bQ\s+METH\b/g, "QMETH")
    .replace(/\bT\s+(HLTH|NURS)\b/g, "T$1")
    .replace(/\s+/g, " ")
    .trim()
...
function getDirectNursingPlanIds() {
  const planner = getPlanner();
  return uniqueSorted(
      ...
      planner
        ...
        .filter((plan) => {
          ...
          return titles.some((value) =>
            /^(?:Nursing|Nursing\s*\(BS\),\s*First Year RN to BSN|Nursing\s*\(BS\),\s*RN to BSN|Nursing\s*\(RN-BSN\))(?:\s|:|\(|$)/i.test(value)
          );
        })
        .map((plan) => plan.id)
    )
...
test("UW nursing complete diagnostic fixture is source scoped and UW-course only", () => {
  assert.deepEqual(
    nursingPrograms.map((program) => program.planId),
    [
      "uw-seattle-nursing",
...
diagnosticTest("UW nursing target roster matches direct official nursing-family majors", () => {
  const expectedPlanIds = uniqueSorted(nursingPrograms.map((program) => program.planId));
  const actualPlanIds = getDirectNursingPlanIds();

  assert.deepEqual(
    actualPlanIds,
    expectedPlanIds,
    [
      "Direct Nursing-family roster should include Seattle Nursing, Bothell First Year RN to BSN, Bothell RN to BSN, and Tacoma RN-BSN.",
      "Public Health, Health Studies, Healthcare Leadership, Biomedical Sciences, Nutrition, and Medical Laboratory Science are adjacent health programs and should stay outside this nursing-family fixture.",
