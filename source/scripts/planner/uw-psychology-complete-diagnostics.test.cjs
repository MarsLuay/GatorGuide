...
const {
  psychologyPrograms,
} = require("./fixtures/uw-psychology-complete-diagnostics.fixture.cjs");

const RUN_DIAGNOSTICS =
  process.env.TRANSFER_PLANNER_RUN_UW_PSYCHOLOGY_DIAGNOSTICS === "1";
...
function normalizeCourseCode(value) {
  return String(value ?? "")
    .replace(/\u00a0/g, " ")
    .toUpperCase()
    .replace(/\bB\s+(BIO|BUS|EDUC|MATH|WRIT)\b/g, "B$1")
    .replace(/\bT\s+(MATH|PSYCH|SOCWF|URB)\b/g, "T$1")
    .replace(/\s+/g, " ")
    .trim()
...
function getDirectPsychologyPlanIds() {
  const planner = getPlanner();
  return uniqueSorted(
...
test("UW psychology complete diagnostic fixture is source scoped and UW-course only", () => {
  assert.deepEqual(
    psychologyPrograms.map((program) => program.planId),
    [
  ...
  for (const program of psychologyPrograms) {
...
diagnosticTest("UW psychology target roster matches direct official psychology majors", () => {
  const expectedPlanIds = uniqueSorted(psychologyPrograms.map((program) => program.planId));
  const actualPlanIds = getDirectPsychologyPlanIds();

  assert.deepEqual(
    actualPlanIds,
    expectedPlanIds,
    [
      "Direct Psychology roster should include Seattle, Bothell, and Tacoma Psychology only.",
      "Adjacent social science, neuroscience, community health, and education programs should stay outside this target fixture.",
