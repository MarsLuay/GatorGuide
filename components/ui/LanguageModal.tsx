  ...
  return (
    <Modal visible={visible} transparent animationType="fade">
      <View className="flex-1 justify-center items-center bg-black/60 px-6">
        <View className="bg-emerald-900 w-full max-w-sm rounded-3xl p-6 border border-emerald-700">
            ...
            <View className="gap-2">
              {LANGUAGES.map((lang) => (
                <Pressable 
                  ...
                  className={`py-4 rounded-xl ${i18n.language === lang.code ? 'bg-emerald-500/10 border border-emerald-500/50' : 'bg-neutral-800/50'}`}
                >
                  <Text className={`text-center text-base ${i18n.language === lang.code ? 'text-emerald-500 font-bold' : 'text-white/90'}`}>
                    {lang.label}
                  </Text>
                </Pressable>
              ))}
