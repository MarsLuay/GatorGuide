...
exports.TRANSFER_PLANNER_SOURCE_SUMMARY = exports.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY_SUMMARY = exports.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY = exports.TRANSFER_PLANNER_REQUIREMENT_SOURCE_ADAPTER_REGISTRY_SUMMARY = exports.TRANSFER_PLANNER_PARSED_REQUIREMENT_SOURCE_BLOCK_REGISTRY = exports.TRANSFER_PLANNER_REQUIREMENT_SOURCE_FINGERPRINT_REGISTRY = exports.TRANSFER_PLANNER_SOURCE_FINGERPRINT_REGISTRY = exports.TRANSFER_PLANNER_SOURCE_GAP_REGISTRY = exports.TRANSFER_PLANNER_SOURCE_MANIFEST_REGISTRY = exports.TRANSFER_PLANNER_MAJOR_PATHWAY_REGISTRY = exports.TRANSFER_PLANNER_POLICY_REGISTRY = exports.TRANSFER_PLANNER_DEGREE_MAP_BLOCK_REGISTRY = exports.TRANSFER_PLANNER_MAJOR_REQUIREMENT_REGISTRY = exports.TRANSFER_PLANNER_EQUIVALENCY_RULE_REGISTRY = exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY = void 0;
exports.getTransferPlannerSourceManifestEntriesForPlan = getTransferPlannerSourceManifestEntriesForPlan;
exports.getTransferPlannerPrimaryDegreeRequirementsSource = getTransferPlannerPrimaryDegreeRequirementsSource;
exports.getTransferPlannerPromotedPrimarySourceOverride = getTransferPlannerPromotedPrimarySourceOverride;
exports.getTransferPlannerPromotedRequirementAtomOverrides = getTransferPlannerPromotedRequirementAtomOverrides;
exports.getTransferPlannerRequirementDiffClassifications = getTransferPlannerRequirementDiffClassifications;
exports.getTransferPlannerParsedRequirementSourceBlocks = getTransferPlannerParsedRequirementSourceBlocks;
exports.getTransferPlannerCanonicalCourse = getTransferPlannerCanonicalCourse;
exports.isTransferPlannerEquivalencyRuleEffectiveForTerm = isTransferPlannerEquivalencyRuleEffectiveForTerm;
exports.getTransferPlannerEquivalencyRulesForSourceCourse = getTransferPlannerEquivalencyRulesForSourceCourse;
...
const course_metadata_1 = require("./course-metadata");
const equivalency_guide_generated_1 = require("./equivalency-guide.generated");
const requirement_source_adapters_generated_1 = require("./requirement-source-adapters.generated");
const source_manifest_primary_overrides_generated_1 = require("./source-manifest-primary-overrides.generated");
const requirement_atom_overrides_generated_1 = require("./requirement-atom-overrides.generated");
const requirement_diff_classifications_generated_1 = require("./requirement-diff-classifications.generated");
const source_gaps_generated_1 = require("./source-gaps.generated");
const source_fingerprints_generated_1 = require("./source-fingerprints.generated");
...
const GRC_AVAILABILITY_SOURCE_LINKS = [
    {
...
const SOURCE_MANIFEST_PRIMARY_OVERRIDE_BY_OWNER = new Map(source_manifest_primary_overrides_generated_1.TRANSFER_PLANNER_PROMOTED_PRIMARY_SOURCE_OVERRIDES.map((entry) => [entry.ownerId, entry]));
const ALL_UW_CAMPUSES = [
    "uw-seattle",
    "uw-bothell",
    "uw-tacoma",
];
...
const AVAILABILITY_QUARTER_LABELS = {
    summer: "Summer",
    fall: "Fall",
    winter: "Winter",
    spring: "Spring",
};
const GUIDE_TERM_ORDER = {
    WIN: 1,
    SPR: 2,
    SUM: 3,
    AUT: 4,
};
const REQUIREMENT_DISPLAY_PHASE_OVERRIDES = {
...
const STRUCTURED_EQUIVALENCY_RULES = [
    {
...
function parseGuideTermSortValue(label) {
...
function isEffectiveRangeActiveForGuideTerm(range, termLabel) {
...
function normalizeExtractedCourseCode(value) {
...
function extractCourseCodes(value) {
    return unique((String(value ?? "").toUpperCase().match(COURSE_CODE_PATTERN) ?? [])
        .map((match) => normalizeExtractedCourseCode(match))
        .filter((match) => Boolean(match)));
}
function extractCourseCodesFromList(values) {
...
function getAlternativeSetKey(codes) {
...
function toSourceLinks(links) {
    return dedupeLinks((links ?? []).map((link) => ({
        label: link.label,
        url: link.url,
        note: link.note,
    })));
}
function getCourseId(schoolId, code) {
...
function parseCourseParts(code) {
...
function getLastValidatedOn(validationNotes) {
...
function getSourceManifestRole(link) {
    const searchable = `${link.label} ${link.url}`.toLowerCase();
    if (/degree requirements|major requirements|graduation requirements|degree structure|degree sheet|requirement sheet|checklist|requirements packet/.test(searchable)) {
...
function getSourceManifestParserType(link, role) {
...
function getSourceManifestConfidence(role, parserType) {
    if (parserType === "annual-schedule-pdf" ||
...
function getSourceManifestPrimaryScore(link) {
    ...
    if (role === "catalog")
        score += 50;
    if (parserType === "pdf-degree-sheet")
        score += 20;
    if (/degree requirements|major requirements|graduation requirements/.test(searchable))
        score += 25;
    if (/curriculum/.test(searchable))
        score += 15;
    if (role === "admissions" || role === "equivalency" || role === "availability")
        score -= 40;
    return score;
}
function pickPrimaryDegreeRequirementsUrl(links) {
...
function createMutableCourseEntry(schoolId, code) {
    const normalizedCode = normalizeCourseCode(code);
...
function getOrCreateCourse(registry, schoolId, code) {
    const normalizedCode = normalizeCourseCode(code);
...
function addCourseReference(registry, params) {
...
function buildEffectiveYearRangesFromLabels(labels) {
    const parsed = labels
    ...
    if (parsed.length > 0) {
        ...
        for (const label of parsed.slice(1)) {
            ...
            ranges.push({
                ...
                note: currentStart.label === currentEnd.label
                    ? undefined
...
function addCoursesFromTerm(registry, params) {
...
function addCoursesFromCatalogYear(registry, track, catalogYear) {
...
function getChecklistSources(plan) {
...
function getPathwaySources(plan, pathway) {
    return {
...
function addPlanChecklistCourses(registry, plan, phase, item) {
...
function addPlanCourseListCourses(registry, plan) {
    const { sourceLinks, validationNotes, lastValidatedOn } = getChecklistSources(plan);
...
function addPlanMasterBankCourses(registry, plan) {
    const { sourceLinks, validationNotes, lastValidatedOn } = getChecklistSources(plan);
    const bankLibrary = new Map(transfer_planner_master_generated_1.TRANSFER_PLANNER_MASTER_BANK_LIBRARY.map((bank) => [bank.id, bank.courses]));
...
function addPlanDegreeMapCourses(registry, plan, section) {
    const { sourceLinks, validationNotes, lastValidatedOn } = getChecklistSources(plan);
...
function applyNormalizedCourseMetadata(registry) {
    for (const metadata of course_metadata_1.TRANSFER_PLANNER_NORMALIZED_COURSE_METADATA) {
...
function finalizeCourseRegistryEntry(entry) {
    const availabilityEntry = entry.schoolId === "grc"
        ? transfer_planner_grc_availability_generated_1.TRANSFER_PLANNER_GRC_COURSE_AVAILABILITY[entry.code]
        : undefined;
    if (availabilityEntry) {
        ...
        if (availabilityEntry.status !== "published-in-latest-schedule" &&
    ...
    return {
        ...
        sourceLinks: Array.from(entry.sourceLinks.values()).sort((left, right) => left.label.localeCompare(right.label)),
        effectiveYearLabels: Array.from(entry.effectiveYearLabels).sort(),
        effectiveYearRanges: Array.from(entry.effectiveYearRanges.values()).sort((left, right) => left.startLabel.localeCompare(right.startLabel) ||
            (left.endLabel ?? "").localeCompare(right.endLabel ?? "")),
        prerequisiteCourseCodes: Array.from(entry.prerequisiteCourseCodes).sort(),
        prerequisiteAlternativeCourseCodeSets: Array.from(entry.prerequisiteAlternativeCourseCodeSets)
            .sort()
        ...
        corequisiteAlternativeCourseCodeSets: Array.from(entry.corequisiteAlternativeCourseCodeSets)
            .sort()
            .map((group) => group.split("||").filter(Boolean)),
...
function buildCourseRegistry() {
    ...
    for (const track of bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_TRACKS) {
    ...
    for (const plan of bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_ALL_MAJOR_PLANS) {
...
function mapChainRuleType(ruleId) {
...
function buildEquivalencyRuleRegistry() {
    const structuredRules = STRUCTURED_EQUIVALENCY_RULES.map((rule) => ({
        id: rule.id,
        type: rule.type,
        title: rule.title,
    ...
    const chainRules = transfer_planner_master_generated_1.TRANSFER_PLANNER_MASTER_CHAIN_LIBRARY.map((chain) => {
    ...
    return [
        ...structuredRules,
        ...equivalency_guide_generated_1.TRANSFER_PLANNER_UW_GRC_EQUIVALENCY_GUIDE_RULES,
        ...chainRules,
...
function buildRequirementAtomRegistry() {
        ...
        for (const { phase, itemsKey } of PHASE_CONFIG) {
            for (const item of plan[itemsKey]) {
                entries.push({
                    ...
                    phase,
                    displayPhase: REQUIREMENT_DISPLAY_PHASE_OVERRIDES[`${plan.id}:${item.id}`] ?? phase,
                    title: item.title,
                    grcCourseCodes: item.grcCourses.map((code) => normalizeCourseCode(code)),
                    alternativeCourseCodeSets: (item.alternatives ?? []).map((group) => group.map((code) => normalizeCourseCode(code))),
            ...
            for (const { phase, itemsKey } of PHASE_CONFIG) {
                for (const item of pathway[itemsKey] ?? []) {
                    entries.push({
                        ...
                        majorTitle: plan.title,
                        phase,
                        displayPhase: REQUIREMENT_DISPLAY_PHASE_OVERRIDES[`${plan.id}:${pathway.id}:${item.id}`] ??
                            phase,
                        title: item.title,
                        grcCourseCodes: item.grcCourses.map((code) => normalizeCourseCode(code)),
                        alternativeCourseCodeSets: (item.alternatives ?? []).map((group) => group.map((code) => normalizeCourseCode(code))),
    ...
    for (const override of requirement_atom_overrides_generated_1.TRANSFER_PLANNER_PROMOTED_REQUIREMENT_ATOM_OVERRIDES) {
        entryMap.set(override.id, {
            ...
            alternativeCourseCodeSets: (override.alternativeCourseCodeSets ?? []).map((group) => group.map((code) => normalizeCourseCode(code))),
...
function pushSourceManifestEntries(entries, params) {
    const promotedPrimary = SOURCE_MANIFEST_PRIMARY_OVERRIDE_BY_OWNER.get(params.ownerId);
    ...
    const mergedValidationNotes = unique(compact([
...
exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY = buildCourseRegistry();
exports.TRANSFER_PLANNER_EQUIVALENCY_RULE_REGISTRY = buildEquivalencyRuleRegistry();
exports.TRANSFER_PLANNER_MAJOR_REQUIREMENT_REGISTRY = buildRequirementAtomRegistry();
exports.TRANSFER_PLANNER_DEGREE_MAP_BLOCK_REGISTRY = buildDegreeMapBlockRegistry();
exports.TRANSFER_PLANNER_POLICY_REGISTRY = buildPolicyRegistry();
exports.TRANSFER_PLANNER_MAJOR_PATHWAY_REGISTRY = buildPathwayRegistry();
exports.TRANSFER_PLANNER_SOURCE_MANIFEST_REGISTRY = buildSourceManifestRegistry();
exports.TRANSFER_PLANNER_SOURCE_GAP_REGISTRY = source_gaps_generated_1.TRANSFER_PLANNER_SOURCE_GAP_ENTRIES;
exports.TRANSFER_PLANNER_SOURCE_FINGERPRINT_REGISTRY = source_fingerprints_generated_1.TRANSFER_PLANNER_SOURCE_FINGERPRINTS;
exports.TRANSFER_PLANNER_REQUIREMENT_SOURCE_FINGERPRINT_REGISTRY = source_fingerprints_generated_1.TRANSFER_PLANNER_REQUIREMENT_SOURCE_FINGERPRINTS;
exports.TRANSFER_PLANNER_PARSED_REQUIREMENT_SOURCE_BLOCK_REGISTRY = requirement_source_adapters_generated_1.TRANSFER_PLANNER_PARSED_REQUIREMENT_SOURCE_BLOCKS;
exports.TRANSFER_PLANNER_REQUIREMENT_SOURCE_ADAPTER_REGISTRY_SUMMARY = requirement_source_adapters_generated_1.TRANSFER_PLANNER_REQUIREMENT_SOURCE_ADAPTER_SUMMARY;
exports.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY = requirement_diff_classifications_generated_1.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATIONS;
exports.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY_SUMMARY = requirement_diff_classifications_generated_1.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_SUMMARY;
const HIDDEN_SOURCE_GAP_PLAN_IDS = new Set(exports.TRANSFER_PLANNER_SOURCE_GAP_REGISTRY.filter((entry) => entry.studentVisibility === "hidden" && !entry.pathwayId).map((entry) => entry.planId));
const HIDDEN_SOURCE_GAP_PATHWAY_KEYS = new Set(exports.TRANSFER_PLANNER_SOURCE_GAP_REGISTRY.filter((entry) => entry.studentVisibility === "hidden" && entry.pathwayId).map((entry) => `${entry.planId}::${entry.pathwayId}`));
const SOURCE_GENERATED_PATHWAY_COUNT = bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_ALL_MAJOR_PLANS.reduce((count, plan) => count + (plan.pathways?.length ?? 0), 0);
const STUDENT_VISIBLE_SOURCE_GENERATED_PATHWAY_COUNT = bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_ALL_MAJOR_PLANS.reduce((count, plan) => {
    if (HIDDEN_SOURCE_GAP_PLAN_IDS.has(plan.id))
        return count;
    return (count +
        (plan.pathways ?? []).filter((pathway) => !HIDDEN_SOURCE_GAP_PATHWAY_KEYS.has(`${plan.id}::${pathway.id}`)).length);
}, 0);
exports.TRANSFER_PLANNER_SOURCE_SUMMARY = {
    generatedOn: "2026-04-02",
    sourceGeneratedMajorPlanCount: bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_ALL_MAJOR_PLANS.length,
    studentVisibleMajorPlanCount: bootstrap_generated_1.TRANSFER_PLANNER_BOOTSTRAP_ALL_MAJOR_PLANS.filter((plan) => !HIDDEN_SOURCE_GAP_PLAN_IDS.has(plan.id)).length,
    ...
    canonicalCourseCount: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.length,
    canonicalCourseTitleCount: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => Boolean(entry.title)).length,
    canonicalCourseCreditCount: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.creditValue !== null).length,
    canonicalCoursePrerequisiteCount: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.prerequisiteCourseCodes.length > 0 ||
        entry.prerequisiteAlternativeCourseCodeSets.length > 0).length,
    canonicalCourseCorequisiteCount: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.corequisiteCourseCodes.length > 0 ||
        entry.corequisiteAlternativeCourseCodeSets.length > 0).length,
    canonicalCourseEffectiveYearRangeCount: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.effectiveYearRanges.length > 0).length,
    equivalencyRuleCount: exports.TRANSFER_PLANNER_EQUIVALENCY_RULE_REGISTRY.length,
    equivalencyGuideParsedRuleCount: exports.TRANSFER_PLANNER_EQUIVALENCY_RULE_REGISTRY.filter((entry) => entry.sourceKind === "uw-green-river-equivalency-guide").length,
    equivalencyRuleCountsBySourceKind: exports.TRANSFER_PLANNER_EQUIVALENCY_RULE_REGISTRY.reduce((counts, entry) => {
        const key = entry.sourceKind ?? "unknown";
        counts[key] = (counts[key] ?? 0) + 1;
        return counts;
    }, {}),
    equivalencyRuleCountsByType: exports.TRANSFER_PLANNER_EQUIVALENCY_RULE_REGISTRY.reduce((counts, entry) => {
        counts[entry.type] = (counts[entry.type] ?? 0) + 1;
        return counts;
    }, {}),
    majorRequirementCount: exports.TRANSFER_PLANNER_MAJOR_REQUIREMENT_REGISTRY.length,
    degreeMapBlockCount: exports.TRANSFER_PLANNER_DEGREE_MAP_BLOCK_REGISTRY.length,
    policyEntryCount: exports.TRANSFER_PLANNER_POLICY_REGISTRY.length,
    majorPathwayCount: exports.TRANSFER_PLANNER_MAJOR_PATHWAY_REGISTRY.length,
    sourceManifestCount: exports.TRANSFER_PLANNER_SOURCE_MANIFEST_REGISTRY.length,
    sourceManifestPrimaryCount: exports.TRANSFER_PLANNER_SOURCE_MANIFEST_REGISTRY.filter((entry) => entry.isPrimaryDegreeRequirementsLink).length,
    sourceManifestHighConfidenceCount: exports.TRANSFER_PLANNER_SOURCE_MANIFEST_REGISTRY.filter((entry) => entry.confidence === "high").length,
    sourceManifestPromotedPrimaryOverrideCount: source_manifest_primary_overrides_generated_1.TRANSFER_PLANNER_PROMOTED_PRIMARY_SOURCE_OVERRIDES.length,
    sourceGapCount: exports.TRANSFER_PLANNER_SOURCE_GAP_REGISTRY.length,
    sourceGapCountsByStatus: exports.TRANSFER_PLANNER_SOURCE_GAP_REGISTRY.reduce((counts, entry) => {
        counts[entry.sourceCoverageStatus] = (counts[entry.sourceCoverageStatus] ?? 0) + 1;
        return counts;
    }, {}),
    sourceFingerprintCount: exports.TRANSFER_PLANNER_SOURCE_FINGERPRINT_REGISTRY.length,
    requirementSourceFingerprintCount: exports.TRANSFER_PLANNER_REQUIREMENT_SOURCE_FINGERPRINT_REGISTRY.length,
    parsedRequirementSourceBlockCount: exports.TRANSFER_PLANNER_PARSED_REQUIREMENT_SOURCE_BLOCK_REGISTRY.length,
    parsedRequirementAtomCandidateCount: exports.TRANSFER_PLANNER_REQUIREMENT_SOURCE_ADAPTER_REGISTRY_SUMMARY
        .parsedRequirementAtomCandidateCount,
    parsedDegreeMapBlockCandidateCount: exports.TRANSFER_PLANNER_REQUIREMENT_SOURCE_ADAPTER_REGISTRY_SUMMARY
        .parsedDegreeMapBlockCandidateCount,
    requirementSourceAdapterCountsById: exports.TRANSFER_PLANNER_REQUIREMENT_SOURCE_ADAPTER_REGISTRY_SUMMARY.countsByAdapterId,
    catalogDescriptionCount: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => Boolean(entry.catalogDescription)).length,
    catalogPrerequisiteNoteCount: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.prerequisiteNotes.length > 0).length,
    catalogCorequisiteNoteCount: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.corequisiteNotes.length > 0).length,
    promotedRequirementAtomOverrideCount: requirement_atom_overrides_generated_1.TRANSFER_PLANNER_PROMOTED_REQUIREMENT_ATOM_OVERRIDES.length,
    requirementDiffClassificationCount: exports.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY.length,
    requirementDiffClassificationCountsByKind: exports.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY_SUMMARY.countsByKind,
    requirementDiffClassificationCountsByCampus: exports.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY_SUMMARY.countsByCampus,
    courseCountsBySchool: {
        grc: exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.schoolId === "grc")
            .length,
        "uw-seattle": exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.schoolId === "uw-seattle").length,
        "uw-bothell": exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.schoolId === "uw-bothell").length,
        "uw-tacoma": exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.filter((entry) => entry.schoolId === "uw-tacoma").length,
    },
};
function getTransferPlannerSourceManifestEntriesForPlan(planId, pathwayId) {
    return exports.TRANSFER_PLANNER_SOURCE_MANIFEST_REGISTRY.filter((entry) => entry.planId === planId &&
        (pathwayId === undefined
            ? true
            : pathwayId === null
                ? !entry.pathwayId
...
function getTransferPlannerPrimaryDegreeRequirementsSource(planId, pathwayId) {
    return (getTransferPlannerSourceManifestEntriesForPlan(planId, pathwayId).find((entry) => entry.isPrimaryDegreeRequirementsLink) ?? null);
}
function getTransferPlannerPromotedPrimarySourceOverride(ownerId) {
    return SOURCE_MANIFEST_PRIMARY_OVERRIDE_BY_OWNER.get(ownerId) ?? null;
}
function getTransferPlannerPromotedRequirementAtomOverrides(planId, pathwayId) {
    return requirement_atom_overrides_generated_1.TRANSFER_PLANNER_PROMOTED_REQUIREMENT_ATOM_OVERRIDES.filter((entry) => entry.planId === planId &&
        (pathwayId === undefined
            ? true
            : pathwayId === null
                ? !entry.pathwayId
...
function getTransferPlannerRequirementDiffClassifications(planId, pathwayId) {
    return exports.TRANSFER_PLANNER_REQUIREMENT_DIFF_CLASSIFICATION_REGISTRY.filter((entry) => entry.planId === planId &&
        (pathwayId === undefined
            ? true
            : pathwayId === null
                ? !entry.pathwayId
...
function getTransferPlannerParsedRequirementSourceBlocks(planId, pathwayId) {
    return exports.TRANSFER_PLANNER_PARSED_REQUIREMENT_SOURCE_BLOCK_REGISTRY.filter((entry) => entry.planId === planId &&
        (pathwayId === undefined
            ? true
            : pathwayId === null
                ? !entry.pathwayId
...
function getTransferPlannerCanonicalCourse(schoolId, code) {
    const normalizedCode = normalizeCourseCode(code);
    return exports.TRANSFER_PLANNER_CANONICAL_COURSE_REGISTRY.find((entry) => entry.schoolId === schoolId && entry.code === normalizedCode);
}
function isTransferPlannerEquivalencyRuleEffectiveForTerm(rule, termLabel) {
    if (rule.effectiveYearRanges.length === 0) {
        return true;
    }
    return rule.effectiveYearRanges.some((range) => isEffectiveRangeActiveForGuideTerm(range, termLabel));
}
function getTransferPlannerEquivalencyRulesForSourceCourse(sourceCourseCode, effectiveTermLabel) {
    const normalizedCode = normalizeCourseCode(sourceCourseCode);
    return exports.TRANSFER_PLANNER_EQUIVALENCY_RULE_REGISTRY.filter((entry) => {
        const hasSourceCourse = (entry.sourceCourseSets ?? []).some((courseSet) => courseSet.includes(normalizedCode));
        if (!hasSourceCourse) {
            return false;
        }
