...
export default function ForgotPasswordPage() {
  ...
  const emailError = useMemo(() => {
    ...
    return trimmed && !isEmailValid(trimmed) ? t("auth.validation.invalid_email") : undefined;
  }, [email, t]);
  ...
  const handleSubmit = async () => {
    ...
    if (!isEmailValid(e)) {
      Alert.alert(t("common.error"), t("auth.validation.invalid_email"));
      return;
    }
      ...
      <ScreenBackground>
        <View className="flex-1 items-center justify-center px-6">
          <View className="w-full max-w-md">
            ...
            <Text className={`text-3xl text-center ${styles.textClass} mb-2`}>
              {t("auth.check_email_title") || "Check Your Email"}
            </Text>
            <Text className={`${styles.secondaryTextClass} text-center mb-8`}>
              {t("auth.check_email_desc", { email: email.trim() }) || `We've sent a password reset link to ${email.trim()}`}
            </Text>
            <View className={`${styles.cardBgClass} border rounded-2xl p-6`}>
              <Text className={`text-sm ${styles.secondaryTextClass} text-center`}>
                {t("auth.check_email_hint") || "Click the link in the email to reset your password. If you don't see it, check your spam folder."}
              </Text>
            </View>
            <Pressable 
        ...
        <View className="flex-1 px-6 py-8">
          <View className="w-full max-w-md self-center">
            ...
            >
              ...
              <Text className={`${styles.secondaryTextClass} ml-2`}>
                {t("auth.back_to_login") || "Back to Login"}
              </Text>
            </Pressable>
