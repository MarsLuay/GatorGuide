...
import { type NotificationPreferences, useAppData } from "./use-app-data";
...
function shouldSyncNotificationForOpportunity(
  opportunity: Opportunity,
  preferences: NotificationPreferences
) {
  if (opportunity.type === OPPORTUNITY_TYPES.collegeDeadline) {
  ...
  if (opportunity.type === OPPORTUNITY_TYPES.internship) {
    return preferences.internships;
  }
...
export function OpportunitiesProvider({ children }: { children: React.ReactNode }) {
  const { isHydrated: isAppHydrated, restoreVersion, state } = useAppData();
  ...
  const notificationOpportunities = useMemo(
    () =>
      opportunities.filter((opportunity) =>
        shouldSyncNotificationForOpportunity(
          opportunity,
          state.notificationPreferences
        )
      ),
    [opportunities, state.notificationPreferences]
  );
  ...
  }, [isAppHydrated, isHydrated, restoreVersion, userKey]);
    ...
    void notificationsService
      ...
      .catch((error) => {
        void errorLoggingService.captureException(error, {
          ...
          source: "use-opportunities",
          metadata: {
            opportunityCount: notificationOpportunities.length,
            notificationsEnabled: !!state.notificationsEnabled,
          },
        });
      });
  }, [isAppHydrated, isHydrated, notificationOpportunities, state.notificationsEnabled, statusById]);
