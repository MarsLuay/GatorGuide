...
} = require("./fixtures/uw-mechanical-engineering-complete-diagnostics.fixture.cjs");

const RUN_DIAGNOSTICS =
  process.env.TRANSFER_PLANNER_RUN_UW_MECHANICAL_ENGINEERING_DIAGNOSTICS === "1";
...
function normalizeCourseCode(value) {
  return String(value ?? "")
    ...
    .replace(/\bB\s+(CHEM|ENGR|ME|PHYS|WRIT)\b/g, "B$1")
    .replace(/\bST\s+MATH\b/g, "STMATH")
    .replace(/\bT\s+(CES|CHEM|EE|ME|MATH|PHYS)\b/g, "T$1")
    .replace(/\bM\s+E\b/g, "ME")
    .replace(/\bE\s+E\b/g, "EE")
    .replace(/\s+/g, " ")
    .trim()
...
function getDirectMechanicalEngineeringPlanIds() {
  const planner = getPlanner();
  return uniqueSorted(
      ...
      planner
        ...
        .filter((plan) => {
          ...
          return [title, shortTitle].some((value) =>
            /^Mechanical Engineering(?:\s|:|\(|$)/i.test(value)
          );
        })
        .map((plan) => plan.id)
    )
...
test("UW mechanical engineering complete diagnostic fixture is source scoped and UW-course only", () => {
  assert.deepEqual(
    mechanicalEngineeringPrograms.map((program) => program.planId),
    [
  ...
  for (const program of mechanicalEngineeringPrograms) {
...
diagnosticTest("UW mechanical engineering target roster matches direct official ME majors", () => {
  const expectedPlanIds = uniqueSorted(mechanicalEngineeringPrograms.map((program) => program.planId));
  const actualPlanIds = getDirectMechanicalEngineeringPlanIds();

  assert.deepEqual(
    actualPlanIds,
    expectedPlanIds,
    [
      "Direct Mechanical Engineering roster should include Seattle, Bothell, and Tacoma only.",
      "Seattle Biomechanics and Mechatronics should be represented as options/pathways, not duplicate direct major rows.",
