...
} = require("./fixtures/uw-civil-environmental-engineering-complete-diagnostics.fixture.cjs");

const RUN_DIAGNOSTICS =
  process.env.TRANSFER_PLANNER_RUN_UW_CIVIL_ENVIRONMENTAL_ENGINEERING_DIAGNOSTICS === "1";
...
function normalizeCourseCode(value) {
  return String(value ?? "")
    ...
    .replace(/\bT\s+(CE|CHEM|EE|MATH|ME|PHYS)\b/g, "T$1")
    .replace(/\s+/g, " ")
    .trim()
...
function getDirectCivilEnvironmentalEngineeringPlanIds() {
  const planner = getPlanner();
  return uniqueSorted(
      ...
      planner
        ...
        .filter((plan) => {
          ...
          return titles.some((title) =>
            /^(?:Civil Engineering|Civil Engineering\s*\(BSCE\)|Environmental Engineering)(?:\s|\(|$)/i.test(title)
          );
        })
        .map((plan) => plan.id)
    )
...
test("UW civil/environmental engineering complete diagnostic fixture is source scoped and UW-course only", () => {
  assert.deepEqual(
    civilEnvironmentalEngineeringPrograms.map((program) => program.planId),
    [
  ...
  for (const program of civilEnvironmentalEngineeringPrograms) {
...
diagnosticTest("UW civil/environmental engineering target roster matches direct official majors", () => {
  const expectedPlanIds = uniqueSorted(
    civilEnvironmentalEngineeringPrograms.map((program) => program.planId)
  );
  const actualPlanIds = getDirectCivilEnvironmentalEngineeringPlanIds();

  assert.deepEqual(
    actualPlanIds,
    expectedPlanIds,
    [
      "Direct Civil/Environmental Engineering roster should include Seattle Civil Engineering, Seattle Environmental Engineering, and Tacoma Civil Engineering.",
      "Construction Management, Environmental Science, Environmental Public Health, and Sustainable Urban Development are adjacent programs and should stay outside this engineering-family fixture.",
